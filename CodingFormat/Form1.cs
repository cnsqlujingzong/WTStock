...
namespace CodingFormat
{
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string[] files = Directory.GetFiles(@"F:\WTStock\wt-web", "*.aspx", SearchOption.AllDirectories);
            ...
            foreach (string item in files)
            {
                ...
                if (extname.ToLower() == ".aspx")
                {
                    ...
                    for (int i = 0; line != null; i++)
                    {
                       ...
                       sb.Append(line.Replace("language=\"C#\"", "language=\"C#\"  CodeFile=\"" + name + ".aspx.cs\"    ").Replace(", wt.web", "") + "\r\n");
                        }
                        else
                        {
                            sb.Append(line + "\r\n");
        ...
        {
            string[] files = Directory.GetFiles(@"E:\workspace\测试TFS\wt-web\Branch\Customer", "*.aspx", SearchOption.AllDirectories);
            ...
            foreach (string item in files)
            {
                ...
                if (extname.ToLower() == ".aspx")
                {
                    File.WriteAllText(item, File.ReadAllText(item, Encoding.Default), Encoding.GetEncoding("GB2312"));
                }
            }
