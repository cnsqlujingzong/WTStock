...
public partial class Branch_Basic_AccountAdm : Page, IRequiresSessionState
{
	...
	{
		int recordCount = 0;
		this.GridView1.DataSource = DALCommon.GetList_HL(1, "Account", "", this.pageSize, this.jsPager.CurrentPageIndex, "DeptID=" + (string)this.Session["Session_wtBranchID"], " ID Desc", out recordCount);
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			accountInfo._Name = FunLibrary.ChkInput(this.tbName.Text);
			accountInfo.Remark = FunLibrary.ChkInput(this.tbRemark.Text);
			accountInfo.DeptID = new int?(int.Parse((string)this.Session["Session_wtBranchID"]));
			if (this.hfRecID.Value == "-1")
			{
				this.hfTemp.Value = accountInfo._Name;
				string str;
				int num2;
				int num = dALAccount.Add(accountInfo, out str, out num2);
				if (num == 0)
				{
			...
			{
				accountInfo.ID = int.Parse(this.hfRecID.Value);
				string chkfld = string.Empty;
				if (accountInfo._Name != this.hfTemp.Value)
				{
					chkfld = "_Name='" + accountInfo._Name + "' and DeptID=" + (string)this.Session["Session_wtBranchID"];
				}
				string str;
				int num = dALAccount.Update(accountInfo, chkfld, out str);
				if (num == 0)
				{
					this.hfRecID.Value = accountInfo.ID.ToString();
				}
				else
				{
					if (num == -1)
	...
	protected void btnMod_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			accountInfo._Name = FunLibrary.ChkInput(this.tbName.Text);
			accountInfo.ID = int.Parse(this.hfRecID.Value);
			accountInfo.Remark = this.tbRemark.Text;
			string chkfld = string.Empty;
			if (accountInfo._Name != this.hfTemp.Value)
			{
				chkfld = "_Name='" + accountInfo._Name + "' and DeptID=" + (string)this.Session["Session_wtBranchID"];
			...
			string str2;
			int num = dALAccount.Update(accountInfo, chkfld, out str2);
			if (num == 0)
			{
				this.hfRecID.Value = accountInfo.ID.ToString();
				this.FillData();
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int num = DALCommon.DeteleData("Account", "[ID]=" + this.hfRecID.Value, out empty);
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
		}
	...
	protected void btnShow_Click(object sender, EventArgs e)
	{
		...
		{
			DALAccount dALAccount = new DALAccount();
			AccountInfo model = dALAccount.GetModel(int.Parse(this.hfRecID.Value));
