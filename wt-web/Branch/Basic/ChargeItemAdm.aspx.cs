...
public partial class Branch_Basic_ChargeItemAdm : Page, IRequiresSessionState
{
	...
	{
		int recordCount = 0;
		this.GridView1.DataSource = DALCommon.GetList_HL(1, "ChargeItem", "[ID],DeptID,_Name,Type=case when Type=0 then '收入' else '支出' end", this.pageSize, this.jsPager.CurrentPageIndex, " DeptID=" + (string)this.Session["Session_wtBranchID"], " ID Desc", out recordCount);
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			chargeItemInfo.DeptID = new int?(int.Parse((string)this.Session["Session_wtBranchID"]));
			...
			else
			{
				...
				{
					chkfld = "_Name='" + chargeItemInfo._Name + "' and DeptID=" + (string)this.Session["Session_wtBranchID"];
				}
				string str;
