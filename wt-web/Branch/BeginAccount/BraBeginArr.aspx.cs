...
public partial class Branch_BeginAccount_BraBeginArr : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "qc_r4"))
				{
	...
	protected void btnSure_Click(object sender, EventArgs e)
	{
		...
		DataTable gridViewSource = this.GridViewSource;
		DataTable dataTable = DALCommon.GetDataList("jc_branch", "", this.ddlSchFld.SelectedValue + "='" + str + "'").Tables[0];
		...
		if (dataTable.Rows.Count > 0)
		{
			...
			for (int i = 0; i < dataTable.Rows.Count; i++)
			{
				...
				{
					DataRow dataRow = gridViewSource.NewRow();
					dataRow[0] = dataTable.Rows[i]["BranchNO"].ToString();
	...
	protected void btnId_Click(object sender, EventArgs e)
	{
		...
		{
			DataTable dataTable = DALCommon.GetDataList("jc_branch", "", " [ID] in(" + text + ") ").Tables[0];
			if (dataTable.Rows.Count > 0)
			{
				this.CollectData();
				for (int i = 0; i < dataTable.Rows.Count; i++)
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		{
			ScriptManager.RegisterClientScriptBlock(this.UpdatePanel2, this.UpdatePanel2.GetType(), "sysinfo", "window.alert('操作失败！请添加网点');$('" + this.tbCon.ClientID + "').select();", true);
		}
		else
		{
			...
			int num = dALBeginArrearage.Update(beginArrearageInfo, int.Parse((string)this.Session["Session_wtBranchID"]), 3, iOperator, out empty);
			if (num == 0)
			{
				this.SysInfo("window.alert('操作成功！期初网点应收应付已建立');");
				this.ClearText();
			}
			else
			{
