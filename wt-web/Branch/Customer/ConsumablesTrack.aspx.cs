...
public partial class Branch_Customer_ConsumablesTrack : Page, IRequiresSessionState
{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		string text = (this.hfParm.Value == "-1") ? this.strParm() : string.Concat(new string[]
		{
			...
			" and datediff(d,getdate(),WDate-7)<=0 and Status=",
		...
		if (this.gvdata.Rows.Count > 0)
		{
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 1)
				{
					...
					{
						HiddenField expr_3F5 = this.hfTbTitle;
						expr_3F5.Value = expr_3F5.Value + "," + text2;
					}
					...
					{
						HiddenField expr_447 = this.hfTbField;
						expr_447.Value = expr_447.Value + "," + dataField;
					}
				}
			}
		}
	...
	{
		string text = " DeptID=" + (string)this.Session["Session_wtBranchID"] + " and datediff(d,getdate(),WDate-7)<=0 ";
		if (this.iflag == 1)
		{
			text = " DeptID=" + (string)this.Session["Session_wtBranchID"] + " and datediff(d,getdate(),WDate-7)<=0 ";
		}
