...
public partial class Branch_Customer_CustomerTrack : Page, IRequiresSessionState
{
	...
	protected void LoadField()
	{
		...
		if (dataTable.Rows.Count > 0)
		{
			...
			templateField.ItemTemplate = new MyTemplatexcusTTb("", DataControlRowType.DataRow);
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		if (this.iflag == 2)
		{
			text = text + " and [ID] in(select [CustomerID] from CustomerTrack a where datediff(d,dateadd(dd,0,getdate()),a.NextTrack)<=0) and TrackOperatorID=" + (string)this.Session["Session_wtUserBID"];
		}
				...
				if (i > 1)
				{
					...
					if (dataField != "ID")
					{
						...
						else
						{
							...
							expr_4E1.Value = expr_4E1.Value + "," + text3;
						}
						...
						{
							HiddenField expr_533 = this.hfTbField;
							expr_533.Value = expr_533.Value + "," + dataField;
						}
					}
				}
			}
