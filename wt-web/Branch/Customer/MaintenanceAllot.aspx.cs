...
public partial class Branch_Customer_MaintenanceAllot : Page, IRequiresSessionState
{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		string text = (this.hfParm.Value == "-1") ? this.strParm() : string.Concat(new string[]
		{
			...
			" and _Date<=getdate() and Status=",
			this.ddlStatus.SelectedItem.Value
		});
		string fldSort = sortExpression + " " + direction;
		if (this.iflag == 1)
		{
			text = ((this.hfParm.Value == "-1") ? this.strParm() : string.Concat(new string[]
			{
				this.hfParm.Value,
				" and DeptID=",
				(string)this.Session["Session_wtBranchID"],
				" and dateadd(d,RemindDay,getdate())>=_Date and Status=",
				this.ddlStatus.SelectedItem.Value
			}));
		}
		...
		if (this.gvdata.Rows.Count > 0)
		{
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 1)
				{
					...
					{
						HiddenField expr_3D0 = this.hfTbTitle;
						expr_3D0.Value = expr_3D0.Value + "," + text2;
					}
					...
					{
						HiddenField expr_422 = this.hfTbField;
						expr_422.Value = expr_422.Value + "," + dataField;
					}
				}
			}
		}
	...
	{
		string text = " DeptID=" + (string)this.Session["Session_wtBranchID"] + " and _Date<=getdate() ";
		if (this.iflag == 1)
		{
			text = " DeptID=" + (string)this.Session["Session_wtBranchID"] + " and dateadd(d,RemindDay,getdate())>=_Date ";
		}
