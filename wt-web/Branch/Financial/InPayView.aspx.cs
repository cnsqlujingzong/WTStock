...
public partial class InPayView : Page, IRequiresSessionState
{
	private int id;
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (model != null)
			{
				...
				lbsya.Text = (lbsyb.Text = "收");
					tbAmount.Text = model.RecMoney.ToString("#0.00");
					tbRealAmount.Text = model.RealRecMoney.ToString("#0.00");
				}
				else
				{
                 
					lbsya.Text = (lbsyb.Text = "付");
					tbAmount.Text = model.DueMoney.ToString("#0.00");
					tbRealAmount.Text = model.RealDueMoney.ToString("#0.00");
				}
				tbYAmount.Text = model.PreMoney.ToString("#0.00");
				tbInvoiceAmount.Text = model.InvoiceAmount.ToString("#0.00");
				tbInvoiceDate.Text = ((model.InvoiceDate == "1900-01-01") ? "" : model.InvoiceDate);
				for (int i = 0; i < ddlChargeStyle.Items.Count; i++)
				{
