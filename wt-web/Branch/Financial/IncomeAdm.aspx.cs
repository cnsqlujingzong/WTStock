...
public partial class Branch_Financial_IncomeAdm : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			DataSet dataList = DALCommon.GetDataList("SysParm", "bHeadBln", "ID=1");
			string text = dataList.Tables[0].Rows[0][0].ToString().Trim();
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		DataTable dataTable;
		if (this.hfHeadBln.Value == "1")
		{
			text += string.Format(" and (DeptID={0} or ViewDept={0})", (string)this.Session["Session_wtBranchID"]);
			dataTable = DALCommon.GetDataList("zk_viewincome", "sum(isnull(RealRecMoney,0)) as Qty,sum(isnull(RealDueMoney,0)) as Amount,sum(isnull(InvoiceAmount,0)) as InvoiceAmount", text).Tables[0];
		}
		else
		{
			text = text + " and DeptID=" + (string)this.Session["Session_wtBranchID"];
			dataTable = DALCommon.GetDataList("zk_incomeexpenses", "sum(isnull(RealRecMoney,0)) as Qty,sum(isnull(RealDueMoney,0)) as Amount,sum(isnull(InvoiceAmount,0)) as InvoiceAmount", text).Tables[0];
		}
		...
		if (this.hfHeadBln.Value == "1")
		{
			list_HL = DALCommon.GetList_HL(1, "zk_viewincome", "", this.pageSize, this.jsPager.CurrentPageIndex, text, fldSort, out recordCount);
		}
		else
		{
			list_HL = DALCommon.GetList_HL(1, "zk_incomeexpenses", "", this.pageSize, this.jsPager.CurrentPageIndex, text, fldSort, out recordCount);
		}
		this.gvbranch.DataSource = list_HL;
			...
			for (int i = 0; i < this.gvbranch.HeaderRow.Cells.Count; i++)
			{
				if (i > 1)
				{
					...
					if (dataField != "ID" && dataField != "")
					{
						...
						{
							HiddenField expr_4F3 = this.hfTbTitle;
							expr_4F3.Value = expr_4F3.Value + "," + text2;
						}
						...
						{
							HiddenField expr_543 = this.hfTbField;
							expr_543.Value = expr_543.Value + "," + dataField;
						}
					}
				}
			}
