...
public partial class Branch_Financial_PayExpense : Page, IRequiresSessionState
{
		...
		if (this.gvdata.Rows.Count > 0)
		{
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 0)
				{
					...
					{
						HiddenField expr_31F = this.hfTbTitle;
						expr_31F.Value = expr_31F.Value + "," + text2;
					}
					...
					{
						HiddenField expr_36F = this.hfTbField;
						expr_36F.Value = expr_36F.Value + "," + dataField;
					}
				}
			}
		}
	...
	{
		string text = " Status='待发放' and DeptID=" + (string)this.Session["Session_wtBranchID"];
