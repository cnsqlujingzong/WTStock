...
public partial class Branch_Financial_SchExpense : Page, IRequiresSessionState
{
	...
	protected void btnExcelDetail_Click(object sender, EventArgs e)
	{
		...
		DataTable dt = DALCommon.GetDataList("v_zk_excel_expensedetail", this.hfTbFieldDetail.Value, strCondition).Tables[0];
		string[] tbTitle = this.hfTbTitleDetail.Value.Split(new char[]
		{
			','
		});
		string empty = string.Empty;
