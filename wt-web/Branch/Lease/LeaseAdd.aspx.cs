...
public partial class Branch_Lease_LeaseAdd : Page, IRequiresSessionState
{
	...
	protected int BillAdd(int bch, out int iTbid, out string strMsg)
	{
		...
		leaseInfo.DeptID = int.Parse((string)this.Session["Session_wtBranchID"]);
		...
		leaseInfo.OperatorID = int.Parse(this.Session["Session_wtUserBID"].ToString());
