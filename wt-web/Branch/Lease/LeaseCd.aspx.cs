...
public partial class Branch_Lease_LeaseCd : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			OtherFunction.BindStocks(this.ddlStock, " bStop=0 and DeptID=" + int.Parse((string)this.Session["Session_wtBranchID"]) + " ");
			DataTable dataSource = DALCommon.GetDataList("StaffList", "[ID],[_Name]", " Status=0 and DeptID=" + int.Parse((string)this.Session["Session_wtBranchID"]) + " ").Tables[0];
