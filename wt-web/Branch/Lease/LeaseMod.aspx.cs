...
public partial class Branch_Lease_LeaseMod : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			OtherFunction.BindStaff(this.ddlOperator, "DeptID=" + int.Parse((string)this.Session["Session_wtBranchID"]) + " and Status=0 and bSeller=1");
			this.AddEmpty();
	...
	protected void CollectData(out bool b)
	{
		b = true;
	...
	{
		bool flag = true;
		this.CollectData(out flag);
		if (!flag)
		{
			this.SysInfo("window.alert('该网点仓库中不存在该商品')");
			this.BindData();
		}
		else
		{
			...
			for (int i = 0; i < gridViewSource.Rows.Count; i++)
			{
				if (i == e.RowIndex)
				{
					if (int.Parse(gridViewSource.Rows[i]["iFlag"].ToString()) == 1)
					{
						...
						{
							HiddenField expr_CF = this.hfdellist;
							expr_CF.Value = expr_CF.Value + "," + gridViewSource.Rows[i]["RecID"].ToString();
						}
					}
				}
			}
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		else if (this.GridViewSource.Rows.Count == 1)
		{
		...
		else
		{
			...
			num3 = dALLease.Update2(leaseInfo, (string)this.Session["Session_wtUserBID"], list2, out str);
			if (num3 == 0)
			{
