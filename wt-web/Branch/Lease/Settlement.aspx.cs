...
public partial class Branch_Lease_Settlement : Page, IRequiresSessionState
{
	...
	{
		string text = " DeptID=" + int.Parse((string)this.Session["Session_wtBranchID"]) + " and ChargeStatus=1 ";
	...
	{
		e.Row.Cells[0].Visible = false;
		if (Branch_Lease_Settlement.leaseType != 2)
		{
	...
	{
		Branch_Lease_Settlement.leaseType = new DALLease().getLeaseType(int.Parse(this.hfRecID.Value));
