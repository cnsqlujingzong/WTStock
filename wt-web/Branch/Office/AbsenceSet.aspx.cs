...
public partial class Branch_Office_AbsenceSet : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "bg_r15"))
				{
					base.Response.Redirect("~/Headquarters/Pur.aspx?p=d");
					base.Response.End();
				}
			}
			this.GridView1.DataSource = DALCommon.GetDataList("AttendanceTime", "ID,DeptID,Week,CONVERT(varchar(100),WorkTime,8) as WorkTime,CONVERT(varchar(100),AfterWorkTime,8) as AfterWorkTime,Remark", " DeptID=" + (string)this.Session["Session_wtBranchID"]);
			this.GridView1.DataBind();
			this.GridView2.DataSource = DALCommon.GetDataList("LateSet", "", " DeptID=" + (string)this.Session["Session_wtBranchID"]);
			this.GridView2.DataBind();
			this.GridView3.DataSource = DALCommon.GetDataList("AbsenceSet", "", " DeptID=" + (string)this.Session["Session_wtBranchID"]);
