...
public partial class Branch_Office_AnnunciateAdm : Page, IRequiresSessionState
{
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int num = int.Parse(this.Session["Session_wtBranchID"].ToString());
		...
		if (model != null)
		{
			if (model.DeptID == num)
			{
				int num2 = DALCommon.DeteleData("OA_Annunciate", "[ID]=" + this.hfRecID.Value, out empty);
				if (num2 == 0)
				{
					this.hfRecID.Value = "-1";
				}
			...
			{
				this.SysInfo("window.alert('操作失败！只允许删除本部门公告');");
			}
		}
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
	}
