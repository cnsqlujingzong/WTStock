...
public partial class Branch_Receive_SndSure : Page, IRequiresSessionState
{
	...
	{
		string text = " DeptID=" + (string)this.Session["Session_wtBranchID"] + " and Status='待发货确认' ";
	...
	protected void btnUCheck_Click(object sender, EventArgs e)
	{
		int iTbid = 0;
		...
		int num = dALRcvSnd.ConfCancel(iTbid, iOperator, DateTime.Now.ToString("yyyy-MM-dd"), out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		if (num == -1)
		{
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		string empty = string.Empty;
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "发货确认", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
