...
public partial class Branch_Services_AddM : Page, IRequiresSessionState
{
	private int id;
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
			...
			for (int i = 0; i < gridViewSourceM.Rows.Count; i++)
			{
				if (int.Parse(gridViewSourceM.Rows[i]["GoodsID"].ToString()) > 0)
				{
					list.Add(new ServicesMaterialInfo
					{
						...
						TaxAmount = decimal.Parse(gridViewSourceM.Rows[i]["TaxAmount"].ToString())
					});
				}
			}
		...
		if (num == 0)
		{
			DbHelperSQL.InsertErrorLogs(2, int.Parse((string)this.Session["Session_wtUserBID"]), this.id, "添加使用备件", 0);
