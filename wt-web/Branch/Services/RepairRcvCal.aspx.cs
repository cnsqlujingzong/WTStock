...
public partial class Branch_Services_RepairRcvCal : Page, IRequiresSessionState
{
	...
	protected void btnSave_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			int num2 = 0;
			int iOperatorid = int.Parse((string)this.Session["Session_wtUserBID"]);
			string text = this.tbDate.Text;
			string str = "";
			int num3 = dALServices.RepairRcvCal(int.Parse((string)this.Session["Session_wtBranchID"]), iOperatorid, strBillID, repairType, this.corpid, text, num, out num2, out str);
			if (num3 == 0)
			{
				if (num > 0m)
				{
					...
					for (int j = 0; j < gridViewSource.Rows.Count; j++)
					{
						...
						if (num4 > 0m)
						{
							if (num2 > 0)
							{
								dALServices.RepairRcvCalMX(int.Parse((string)this.Session["Session_wtBranchID"]), iOperatorid, iTbid, repairType, this.corpid, text, num4, num2);
							}
						}
					}
				}
