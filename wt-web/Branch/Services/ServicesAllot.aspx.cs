...
public partial class Branch_Services_ServicesAllot : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			this.purid = int.Parse((string)this.Session["Session_wtPurBID"]);
			if (this.purid > 0)
			{
				DALRight dALRight = new DALRight();
				if (!dALRight.GetRight(this.purid, "gd_r15"))
				{
					this.btnPrint.Visible = false;
				}
				if (!dALRight.GetRight(this.purid, "gd_r14"))
				{
					this.btnExcel.Enabled = false;
				}
				if (!dALRight.GetRight(this.purid, "gd_r19"))
				{
					this.btnCancel.Disabled = true;
				}
				if (!dALRight.GetRight(this.purid, "gd_r5"))
				{
					base.Response.Redirect("~/Headquarters/Pur.aspx?p=11");
					base.Response.End();
				}
				if (!dALRight.GetRight(this.purid, "gd_r4"))
				{
				...
				if (dALRight.GetRight(this.purid, "gd_r21"))
				{
				...
				if (dALRight.GetRight(this.purid, "gd_r30"))
				{
	...
	protected void LoadField()
	{
		...
		DataTable dataTable = dALSys.GetLayoutDetail(1, deptID, 7, 0, userID).Tables[0];
		...
		if (dataTable.Rows.Count > 0)
		{
			...
			templateField.ItemTemplate = new MyTemplatexb("", DataControlRowType.DataRow);
			templateField.HeaderTemplate = new MyTemplatexb("", DataControlRowType.Header);
	...
	protected void FillData(string sortExpression, string direction)
	{
		this.LoadField();
		int recordCount = 0;
		string text = (this.hfParm.Value == "-1") ? this.strParm() : (this.hfParm.Value + " and DisposalID=" + (string)this.Session["Session_wtBranchID"] + " and (curStatus='处理中' or curStatus='待处理') ");
			...
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 3)
				{
					...
					if (dataField != "ID" && dataField != "bTake" && dataField != "IsDismissed")
					{
						...
						{
							HiddenField expr_4E1 = this.hfTbTitle;
							expr_4E1.Value = expr_4E1.Value + "," + text2;
						}
						...
						{
							HiddenField expr_531 = this.hfTbField;
							expr_531.Value = expr_531.Value + "," + dataField;
						}
					}
				}
			}
	...
	{
		string text = " DisposalID=" + (string)this.Session["Session_wtBranchID"] + " and (curStatus='处理中' or curStatus='待处理') ";
	...
	protected void btnDelS_Click(object sender, EventArgs e)
	{
		string empty = string.Empty;
		DALServices dALServices = new DALServices();
		if (int.Parse(this.Session["Session_wtPurBID"].ToString()) > 0)
		{
		...
		int num = DALCommon.DeteleData("ServicesProcess", "[ID]=" + this.hfRecID2.Value.Replace("s", ""), out empty);
		if (num == 0)
		{
