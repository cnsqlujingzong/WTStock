...
public partial class Branch_Services_ServicesRcv : Page, IRequiresSessionState
{
	...
	protected void LoadField()
	{
		...
		DataTable dataTable = dALSys.GetLayoutDetail(1, deptID, 9, 0, userID).Tables[0];
		if (dataTable.Rows.Count > 0)
		{
			...
			templateField.ItemTemplate = new MyTemplaterb("", DataControlRowType.DataRow);
	...
	protected void FillData(string sortExpression, string direction)
	{
		this.LoadField();
		int recordCount = 0;
		string text = (this.hfParm.Value == "-1") ? this.strParm() : (this.hfParm.Value + " and DisposalID=" + (string)this.Session["Session_wtBranchID"] + " and curStatus='送修' and RepairStatus='待送修收货' ");
			...
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 5)
				{
					...
					if (dataField != "ID")
					{
						...
						{
							HiddenField expr_2F4 = this.hfTbTitle;
							expr_2F4.Value = expr_2F4.Value + "," + text2;
						}
						...
						{
							HiddenField expr_344 = this.hfTbField;
							expr_344.Value = expr_344.Value + "," + dataField;
						}
					}
				}
			}
	...
	{
		string text = " DisposalID=" + (string)this.Session["Session_wtBranchID"] + " and curStatus='送修' and RepairStatus='待送修收货' ";
