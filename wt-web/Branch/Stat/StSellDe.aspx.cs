...
public partial class Branch_Stat_StSellDe : Page, IRequiresSessionState
{
	...
	{
		int recordCount = 0;
		string text = (this.hfParm.Value == "-1") ? this.strParm() : (this.hfParm.Value + " and DeptID=" + (string)this.Session["Session_wtBranchID"]);
		...
		if (this.gvdata.Rows.Count > 0)
		{
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				...
				if (dataField != "ID" && dataField != "Deduct")
				{
					...
					{
						HiddenField expr_410 = this.hfTbTitle;
						expr_410.Value = expr_410.Value + "," + text2;
					}
					...
					{
						HiddenField expr_462 = this.hfTbField;
						expr_462.Value = expr_462.Value + "," + dataField;
					}
				}
			}
		}
	...
	protected void gvdata_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.DataRow)
		{
			...
			string[] array = e.Row.Cells[22].Text.Split(new char[]
			{
				','
			});
			if (array.Length > 1)
			{
				e.Row.Cells[22].Text = "<a href=\"#\" style=\"color:#0000ff;\" onclick=\"ViewSN('" + e.Row.Cells[22].Text + "');\">查看序列号</a>";
			}
			decimal.TryParse(e.Row.Cells[7].Text, out this.xianjieAmount);
			this.xianjieAmount1 += this.xianjieAmount;
			decimal.TryParse(e.Row.Cells[17].Text, out this.count);
			this.count1 += this.count;
			decimal.TryParse(e.Row.Cells[19].Text, out this.amount);
			this.amount1 += this.amount;
			decimal.TryParse(e.Row.Cells[20].Text, out this.totalAmount);
			this.totalAmount1 += this.totalAmount;
			decimal.TryParse(e.Row.Cells[21].Text, out this.costPrice);
			this.costPrice1 += this.costPrice;
			decimal.TryParse(e.Row.Cells[22].Text, out this.maoli);
			this.maoli1 += this.maoli;
			decimal.TryParse(e.Row.Cells[23].Text, out this.ticheng);
			this.ticheng1 += this.ticheng;
		}
		if (e.Row.RowType == DataControlRowType.Footer)
		{
			e.Row.Cells[1].Text = "合计";
			e.Row.Cells[1].Attributes.Add("style", "padding:0px;background:#ECE9D8;text-align:center;");
			e.Row.Cells[7].Text = this.xianjieAmount1.ToString("#0.00");
			e.Row.Cells[17].Text = this.count1.ToString("#0.00");
			e.Row.Cells[19].Text = this.amount1.ToString("#0.00");
			e.Row.Cells[20].Text = this.totalAmount1.ToString("#0.00");
			e.Row.Cells[21].Text = this.costPrice1.ToString("#0.00");
			e.Row.Cells[22].Text = this.maoli1.ToString("#0.00");
			e.Row.Cells[23].Text = this.ticheng1.ToString("#0.00");
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		DataTable dt = DALCommon.GetDataList("st_selldetail", this.hfTbField.Value, strCondition).Tables[0];
