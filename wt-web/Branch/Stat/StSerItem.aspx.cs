...
public partial class Branch_Stat_StSerItem : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "tj_r25"))
				{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvdata.DataSource = DALCommon.GetStData("tj_fw_xmhz", 0, text, strOrderFld);
		...
		if (this.gvdata.Rows.Count > 0)
		{
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				...
				{
					HiddenField expr_2B3 = this.hfTbTitle;
					expr_2B3.Value = expr_2B3.Value + "," + text2;
				}
				...
				{
					HiddenField expr_303 = this.hfTbField;
					expr_303.Value = expr_303.Value + "," + dataField;
				}
			}
		}
	}
	...
	{
		DataTable dt = DALCommon.GetStData("tj_fw_xmhz", 0, this.hfSql.Value, this.hfOrderName.Value + " " + this.hfOrder.Value).Tables[0];
