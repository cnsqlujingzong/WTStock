...
public partial class Branch_Stock_AllocateAppChk : Page, IRequiresSessionState
{
	...
	{
		string text = " ToDeptID=" + (string)this.Session["Session_wtBranchID"] + " and Status=1 ";
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "调拨审核", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
	...
	{
		DataTable dt = DALCommon.GetDataList("ck_allocatedetail", "GoodsNO,_Name,Spec,ProductBrand,Unit,AppQty,Price,Total,MainTenancePeriod,Remark", " BillID=" + this.hfRecID.Value).Tables[0];
