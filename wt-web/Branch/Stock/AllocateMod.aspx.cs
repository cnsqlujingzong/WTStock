...
public partial class Branch_Stock_AllocateMod : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			OtherFunction.BindStaff(this.ddlOperator, string.Concat(new object[]
			{
				"(DeptID=",
			...
			if (model != null)
			{
				...
				if (model.FromDept != int.Parse((string)this.Session["Session_wtBranchID"]) && model.PersonID != int.Parse(this.Session["Session_wtUserBID"].ToString()))
				{
					this.btnClean.Enabled = false;
					this.btnAdd.Enabled = false;
					this.lbMod.Text = "只能修改本部门的调拨单.";
					this.lbMod.Attributes.Add("class", "si2");
				}
			}
		}
