...
public partial class Branch_Stock_DisBiling : Page, IRequiresSessionState
{
	...
	protected void btnchkgoods_Click(object sender, EventArgs e)
	{
		...
		if (dataTable.Rows.Count > 0)
		{
			...
			{
				string text = this.ddlStock.Items[1].Text;
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num == 0)
			{
				...
				dALDisassembling.ChkDis(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
	...
	protected int BillAdd(out int iTbid)
	{
		...
		disassemblingInfo.DeptID = int.Parse((string)this.Session["Session_wtBranchID"]);
		...
		disassemblingInfo.PersonID = int.Parse(this.Session["Session_wtUserBID"].ToString());
