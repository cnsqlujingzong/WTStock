...
public partial class Branch_Stock_DisBilingMod : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (model2 != null)
			{
				...
				if (model2.DeptID != int.Parse((string)this.Session["Session_wtBranchID"]))
				{
					this.btnClean.Enabled = false;
					this.btnAdd.Enabled = false;
					this.btnChk.Enabled = false;
					this.lbMod.Text = "只能修改本部门的拆装单.";
					this.lbMod.Attributes.Add("class", "si2");
				}
			}
		}
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num == 0)
			{
				...
				dALDisassembling.ChkDis(int.Parse((string)this.Session["Session_wtBranchID"]), this.id, iOperator, out str);
