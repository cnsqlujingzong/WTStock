...
public partial class Branch_Stock_GoodsAdd : Page, IRequiresSessionState
{
	private string f;
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			int purid = int.Parse((string)this.Session["Session_wtPurBID"]);
			DALRight dALRight = new DALRight();
			if (!dALRight.BranchAddGoods(purid, int.Parse(this.Session["Session_wtBranchID"].ToString())))
			{
			...
			OtherFunction.BindSupplier(this.ddlProvider, "bSupplier=1 and bStop=0 ");
			this.BindStockList();
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		goodsInfo.BrandID = new int?(int.Parse(this.ddlBrand.SelectedValue));
		...
		goodsInfo.ProvID = new int?(int.Parse(this.ddlProvider.SelectedValue));
		int value = 0;
		int.TryParse(this.tbValid.Text, out value);
		goodsInfo.Valid = new int?(value);
	...
	protected void ClearText()
	{
		...
		this.ddlProvider.ClearSelection();
		this.ddlProvider.Items.FindByText("").Selected = true;
	...
	protected void btnRefSupplier_Click(object sender, EventArgs e)
	{
		if (this.hfSupplier.Value != string.Empty)
		{
			...
			this.ddlProvider.ClearSelection();
			this.ddlProvider.Items.FindByText(this.hfSupplier.Value).Selected = true;
		}
	}
