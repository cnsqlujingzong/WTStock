...
public partial class Branch_Stock_InnerInChk : Page, IRequiresSessionState
{
	...
	{
		string text = " Status=2 and DeptID=" + int.Parse((string)this.Session["Session_wtBranchID"]);
		int num = int.Parse((string)this.Session["Session_wtPurBID"]);
		if (num > 0 && this.hfSeparate.Value == "1")
		{
			object obj = text;
			text = string.Concat(new object[]
			{
				obj,
				" and StockInID in(select StockID from dbo.getStocks(",
				int.Parse(this.Session["Session_wtUserBID"].ToString()),
				")) "
			});
		}
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		int num = dALInnerAllocate.ChkInnerStockIn(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		string text = "";
		if (num != 0)
		{
	...
	protected void GridView2_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.DataRow)
		{
			...
			string[] array = e.Row.Cells[10].Text.Split(new char[]
			{
				','
			});
			if (array.Length > 1)
			{
				e.Row.Cells[10].Text = "<a href=\"#\" onclick=\"ViewSN('" + e.Row.Cells[10].Text + "');\">查看序列号</a>";
			}
		}
		if (e.Row.RowType == DataControlRowType.Footer)
		{
	...
	{
		DataTable dt = DALCommon.GetDataList("ck_innerallocatedetail", "GoodsNO,_Name,Spec,ProductBrand,Unit,Qty,Price,Total,SN,MainTenancePeriod,Remark", " BillID=" + this.hfRecID.Value).Tables[0];
