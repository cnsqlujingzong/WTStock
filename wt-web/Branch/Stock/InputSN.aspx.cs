...
public partial class Branch_Stock_InputSN : Page, IRequiresSessionState
{
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			...
			DataTable dataTable2 = DALCommon.GetDataList("BranchList", "_Name", "[ID]=" + (string)this.Session["Session_wtBranchID"] + "").Tables[0];
			string dept = dataTable2.Rows[0]["_Name"].ToString();
			int num3 = dALStockSN.InputSN(iOperator, sN, 1, goodsNO, dept, stock, dPrice, out str);
			if (num3 == 0)
			{
				num2++;
			}
