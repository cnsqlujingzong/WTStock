...
public partial class Branch_Stock_SNStockIN : Page, IRequiresSessionState
{
	...
	private DataTable GridViewSourceSN
	{
		get
		{
			if (this.ViewState["ListSN"] == null)
			{
				...
				dataTable.Columns.Add(new DataColumn("bSys", typeof(bool)));
				dataTable.Columns.Add(new DataColumn("ID", typeof(int)));
	...
	protected void gvSN_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.DataRow)
		{
			...
			e.Row.Cells[1].Attributes.Add("style", "width:20px;padding-right:0px;background:#ECE9D8;");
			e.Row.Cells[3].Attributes.Add("style", "padding:0px;text-align:center;");
			CheckBox checkBox = e.Row.FindControl("cbSys") as CheckBox;
			TextBox textBox = e.Row.FindControl("tbSN") as TextBox;
	...
	private void BindDataSN()
	{
		...
		for (int i = 0; i < array2.Length; i++)
		{
			...
			dataRow2[1] = dataRow["bSys"].ToString();
			dataRow2[2] = dataRow["ID"].ToString();
			dataRow2[3] = dataRow["RecID"].ToString();
	...
	protected void btnId_Click(object sender, EventArgs e)
	{
		...
		{
			DataTable dataTable = DALCommon.GetDataList("sn_stockindetail", "", " [ID] in(" + text + ") ").Tables[0];
				...
				for (int i = 0; i < dataTable.Rows.Count; i++)
				{
					...
					if (flag)
					{
						...
						while (num2 < num)
						{
							...
							dataRow2[0] = "";
							dataRow2[1] = false;
							dataRow2[2] = int.Parse(dataTable.Rows[i]["ID"].ToString());
							dataRow2[3] = gridViewSourceSN.Rows.Count;
							gridViewSourceSN.Rows.Add(dataRow2);
							num2++;
						}
					}
	...
	private void CollectData()
	{
		...
		for (int i = 0; i < this.gvSN.Rows.Count; i++)
		{
			...
			CheckBox checkBox = this.gvSN.Rows[i].FindControl("cbSys") as CheckBox;
			int index = 0;
			...
			gridViewSourceSN2.Rows[index]["bSys"] = checkBox.Checked;
		}
	}
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		{
			this.SysInfo("window.alert('操作失败！请添加入库明细');");
		}
		else
		{
			DataTable gridViewSource = this.GridViewSource;
			if (!this.cbSys.Checked)
			{
				for (int i = 0; i < gridViewSource.Rows.Count; i++)
				{
					...
					for (int j = 0; j < array2.Length; j++)
					{
						...
						if (!bool.Parse(dataRow["bSys"].ToString()))
						{
			...
			for (int i = 0; i < gridViewSource.Rows.Count; i++)
			{
				...
				for (int j = 0; j < array2.Length; j++)
				{
					...
					array3[0] = dataRow["ID"].ToString();
					array3[1] = dataRow["SN"].ToString();
					if (this.cbSys.Checked)
					{
						array3[2] = "true";
					}
					...
					{
						array3[2] = dataRow["bSys"].ToString();
					}
					array3[3] = (string)this.Session["Session_wtBranchID"];
					list.Add(array3);
				}
			}
			DALStockSN dALStockSN = new DALStockSN();
			int num = dALStockSN.Add(list);
			if (num == 0)
			{
