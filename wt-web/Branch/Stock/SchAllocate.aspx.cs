...
public partial class Branch_Stock_SchAllocate : Page, IRequiresSessionState
{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		if (this.iflag == 1)
		{
			text = text + " and FromDeptID=" + (string)this.Session["Session_wtBranchID"] + " and Status=6 ";
		}
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		if (model != null)
		{
			if (model.FromDept == int.Parse((string)this.Session["Session_wtBranchID"]))
			{
				if (model.Status == 6)
				{
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int num = dALAllocate.Delete(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
