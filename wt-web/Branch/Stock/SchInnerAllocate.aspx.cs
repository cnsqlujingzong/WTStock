...
public partial class Branch_Stock_SchInnerAllocate : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "ck_r69"))
				{
					base.Response.Redirect("~/Headquarters/Pur.aspx?p=11");
					base.Response.End();
				}
				if (!dALRight.GetRight(num, "ck_r71"))
				{
				...
				if (!dALRight.GetRight(num, "ck_r74"))
				{
	...
	protected string strParm()
	{
		...
		if (num > 0)
		{
			text += string.Format(" and (StockOutID in(select StockID from dbo.getStocks({0})) or StockInID in(select StockID from dbo.getStocks({0})))", this.Session["Session_wtUserBID"].ToString());
		}
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		int num = dALInnerAllocate.ChkStockOut(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		string text = "";
		if (num != 0)
		{
	...
	protected void btnChkU_Click(object sender, EventArgs e)
	{
		...
		int.TryParse((string)this.Session["Session_wtUserBID"], out iOperator);
		int num;
		if (this.hfStatus.Value.IndexOf("已入库审核") >= 0)
		{
			num = dALInnerAllocate.ChkInnerStockInU(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		}
		else
		{
			num = dALInnerAllocate.ChkInnerStockOutU(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		}
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		string text = "";
		if (num != 0)
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int iOperator = 0;
		int.TryParse((string)this.Session["Session_wtUserBID"], out iOperator);
		int num = dALInnerAllocate.Delete(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		string empty = string.Empty;
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "内部调拨", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
