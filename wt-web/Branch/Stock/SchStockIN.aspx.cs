...
public partial class Branch_Stock_SchStockIN : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "ck_r8"))
				{
				...
				if (!dALRight.GetRight(num, "ck_r10"))
				{
					this.btnChkU.Enabled = false;
				}
				if (!dALRight.GetRight(num, "ck_r12"))
				{
					this.btnDel.Enabled = false;
				}
				if (!dALRight.GetRight(num, "ck_r13"))
				{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvdata.DataSource = DALCommon.GetList_HL(1, "ck_stockin", "", this.pageSize, this.jsPager.CurrentPageIndex, text, fldSort, out recordCount);
		...
		if (this.hfSch.Value == "0")
		{
			if (this.ddlKey.SelectedValue != "-1")
			{
				...
				if (text2 != "")
				{
					...
					text += dALStockIN.GetSchWhere(schid, text2);
				}
			}
		}
		return text;
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		int num = dALStockIN.ChkStockIn(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		string text = "";
		if (num != 0)
		{
	...
	protected void btnChkU_Click(object sender, EventArgs e)
	{
		...
		int iOperator = 0;
		int.TryParse((string)this.Session["Session_wtUserBID"], out iOperator);
		int num = dALStockIN.ChkStockInU(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		string text = "";
		if (num != 0)
		{
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int iOperator = 0;
		int.TryParse((string)this.Session["Session_wtUserBID"], out iOperator);
		int num = dALStockIN.Delete(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		DataTable dt = DALCommon.GetDataList("ck_stockin", this.hfTbField.Value, strCondition).Tables[0];
	...
	{
		this.GridView2.DataSource = DALCommon.GetDataList("ck_stockindetail", "", " BillID=" + this.hfRecID.Value);
		this.GridView2.DataBind();
	}

	protected void btnEx_Click(object sender, EventArgs e)
	{
		DataTable dt = DALCommon.GetDataList("ck_stockindetail", "StockName,GoodsNO,_Name,Spec,Attr,ProductBrand,Unit,Qty,Price,Total,SN,MainTenancePeriod,Remark", " BillID=" + this.hfRecID.Value).Tables[0];
