...
public partial class Branch_Stock_SchStockOut : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "ck_r16"))
				{
					base.Response.Redirect("~/Headquarters/Pur.aspx?p=11");
					base.Response.End();
				}
				if (!dALRight.GetRight(num, "ck_r17"))
				{
					this.btnChk.Enabled = false;
				}
				if (!dALRight.GetRight(num, "ck_r18"))
				{
					this.btnChkU.Enabled = false;
				}
				if (!dALRight.GetRight(num, "ck_r20"))
				{
				...
				if (!dALRight.GetRight(num, "ck_r22"))
				{
					this.btnPrint.Visible = false;
				}
				if (dALRight.GetRight(num, "ck_r83"))
				{
					this.hfvalue.Value = "0";
				}
			}
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvdata.DataSource = DALCommon.GetList_HL(1, "ck_stockout", "", this.pageSize, this.jsPager.CurrentPageIndex, text, fldSort, out recordCount);
		...
		if (this.hfSch.Value == "0")
		{
			if (this.ddlKey.SelectedValue != "-1")
			{
				...
				if (text2 != "")
				{
					...
					text += dALStockOUT.GetSchWhere(schid, text2);
				}
			}
		}
		return text;
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		int num = dALStockOUT.ChkStockOut(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		string text = "";
		if (num != 0)
		{
	...
	protected void btnChkU_Click(object sender, EventArgs e)
	{
		...
		int iOperator = 0;
		int.TryParse((string)this.Session["Session_wtUserBID"], out iOperator);
		int num = dALStockOUT.ChkStockOutU(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		string text = "";
		if (num != 0)
		{
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int iOperator = 0;
		int.TryParse((string)this.Session["Session_wtUserBID"], out iOperator);
		int num = dALStockOUT.Delete(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		DataTable dt = DALCommon.GetDataList("ck_stockout", this.hfTbField.Value, strCondition).Tables[0];
		...
		string empty = string.Empty;
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "出库单", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
	...
	{
		this.GridView2.DataSource = DALCommon.GetDataList("ck_stockoutdetail", "", " BillID=" + this.hfRecID.Value);
		this.GridView2.DataBind();
	}

	protected void btnEx_Click(object sender, EventArgs e)
	{
		DataTable dt = DALCommon.GetDataList("ck_stockoutdetail", "StockName,GoodsNO,_Name,Attr,Spec,ProductBrand,Unit,Qty,Price,Total,SN,MainTenancePeriod,Remark", " BillID=" + this.hfRecID.Value).Tables[0];
		...
		string empty = string.Empty;
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "入库单", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
			this.SysInfo("window.alert(\"" + empty + "\");");
