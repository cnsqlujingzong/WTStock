...
public partial class Branch_Stock_SltDisbGoods : Page, IRequiresSessionState
{
	...
	protected void LoadClass()
	{
		...
		Branch_Stock_SltDisbGoods.BindTreeNode(treeNode, dt, treeNode.Value);
	...
	public static void BindTreeNode(TreeNode node, DataTable dt, string parentid)
	{
		...
		for (int i = 0; i < array2.Length; i++)
		{
			...
			Branch_Stock_SltDisbGoods.BindTreeNode(treeNode, dt, dataRow["ID"].ToString());
		}
	}
	...
	protected void gvgds_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.DataRow)
		{
			...
			e.Row.Attributes.Add("onclick", string.Concat(new string[]
			{
				...
				e.Row.Cells[0].Text,
				"','",
				e.Row.Cells[2].Text.Replace("'", "").Replace("\"", "").Replace("&nbsp;", ""),
				"');"
			}));
