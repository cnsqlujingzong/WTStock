...
public partial class Branch_Stock_SltGoodsChk : Page, IRequiresSessionState
{
	...
	protected void LoadClass()
	{
		...
		Branch_Stock_SltGoodsChk.BindTreeNode(treeNode, dt, treeNode.Value);
	...
	public static void BindTreeNode(TreeNode node, DataTable dt, string parentid)
	{
		...
		for (int i = 0; i < array2.Length; i++)
		{
			...
			Branch_Stock_SltGoodsChk.BindTreeNode(treeNode, dt, dataRow["ID"].ToString());
		}
	}
	...
	{
		string text = " bStop=0 and DeptID=" + (string)this.Session["Session_wtBranchID"];
