...
public partial class Branch_Stock_StockAdm : Page, IRequiresSessionState
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (RightID > 0)
        {
            ...
            if (!dalRight.GetRight(RightID, "ck_r1"))
            {
                this.Response.Redirect("~/Headquarters/Pur.aspx?p=5");
                this.Response.End();
            }
            if (dalRight.GetRight(RightID, "jc_r12"))
                this.hfPurCost.Value = "1";
            if (dalRight.GetRight(RightID, "jc_r13"))
        ...
        {
            DALRight dalRight = new DALRight();
            if (!dalRight.GetRight(RightID, "ck_r81"))
            {
                this.ddlBranch.Items.Clear();
                this.ddlBranch.Items.Add(new ListItem("总部", "1"));
                this.ddlBranch.Items.Add(new ListItem((string)this.Session["Session_wtBranch"], (string)this.Session["Session_wtBranchID"]));
                this.ddlBranch.SelectedValue = (string)this.Session["Session_wtBranchID"];
            }
            if (!dalRight.GetRight(RightID, "ck_r80"))
            {
    ...
    protected void LoadClass()
    {
        ...
        Branch_Stock_StockAdm.BindTreeNode(treeNode, dt, treeNode.Value);
    ...
    public static void BindTreeNode(TreeNode node, DataTable dt, string parentid)
    {
        foreach (DataRow dataRow in dt.Select(" Father=" + parentid))
        {
            ...
            Branch_Stock_StockAdm.BindTreeNode(treeNode, dt, dataRow["ID"].ToString());
        }
    }
    ...
    protected void LoadField()
    {
        ...
        DataTable dataTable = new DALSys().GetLayoutDetail(1, result2, 2, 0, result1).Tables[0];
    ...
    protected void btnExcel_Click(object sender, EventArgs e)
    {
        ...
        bool iFlag = false;
        DataToExcel.DataTableToExcel(dt, TbTitle, Guid.NewGuid().ToString() + ".xls", "当前库存", out iFlag, out result);
        if (iFlag)
            return;
        this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
    ...
    protected void GetStockDept()
    {
        ...
        int RightID = int.Parse((string)this.Session["Session_wtPurBID"]);
        if (RightID > 0 && new DALRight().GetRight(RightID, "ck_r78"))
            str = str + (object)" and (StockAdmID1=" + (string)(object)result + " or StockAdmID2=" + (string)(object)result + " or charindex('" + this.Session["Session_wtUserB"].ToString() + "',OtherStockAdm)>0) ";
        this.GridView1.DataSource = (object)DALCommon.GetDataList("ck_stockdept", "", "bReject=0 and GoodsID=" + this.hfRecID.Value + str).Tables[0];
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        double.TryParse(e.Row.Cells[5].Text, out result2);
        double.TryParse(e.Row.Cells[6].Text, out result3);
        if (result1 < result2)
