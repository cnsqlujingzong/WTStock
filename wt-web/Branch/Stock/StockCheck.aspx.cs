...
public partial class Branch_Stock_StockCheck : Page, IRequiresSessionState
{
			...
			if (dataTable.Rows.Count > 0)
			{
				...
				for (int i = 0; i < dataTable.Rows.Count; i++)
				{
					...
					if (flag)
					{
						...
						{
							dataRow[6] = int.Parse(array[i].ToString());
						}
						dataRow[7] = "";
						dataRow[8] = int.Parse(dataTable.Rows[i]["StockLocID"].ToString());
						dataRow[9] = int.Parse(dataTable.Rows[i]["ID"].ToString());
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num == 0)
			{
				...
				dALStockCheck.ChkStockCheck(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
	...
	protected int BillAdd(out int iTbid)
	{
		...
		stockCheckInfo.DeptID = int.Parse((string)this.Session["Session_wtBranchID"]);
