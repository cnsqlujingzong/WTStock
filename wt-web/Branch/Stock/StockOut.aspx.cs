...
public partial class Branch_Stock_StockOut : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			this.tbBillID.Text = DALCommon.CreateID(9, 0);
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num == 0)
			{
				...
				dALStockOUT.ChkStockOut(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num == 0)
			{
				this.SysInfo("window.alert('操作成功！该出库单已保存');");
				this.ClearText();
			}
			else
			{
	...
	protected int BillAdd(out int iTbid)
	{
		...
		stockOUTInfo.DeptID = new int?(int.Parse((string)this.Session["Session_wtBranchID"]));
		...
		stockOUTInfo.PersonID = new int?(int.Parse(this.Session["Session_wtUserBID"].ToString()));
		stockOUTInfo.Remark = FunLibrary.ChkInput(this.tbRemark.Text);
		stockOUTInfo.Status = new int?(1);
		stockOUTInfo.Type = "其他出库";
		DataTable gridViewSource = this.GridViewSource;
		...
		DALStockOUT dALStockOUT = new DALStockOUT();
		int result = dALStockOUT.Add(stockOUTInfo, out iTbid);
