...
public partial class Branch_System_DeviceSExpire : Page, IRequiresSessionState
{
	...
	{
		string text = " DeptID=" + (string)this.Session["Session_wtBranchID"] + " and Warranty='维修保修期' ";
		if (this.ddlType.SelectedValue == "0")
		{
			text += " and datediff(d,getdate(),RepairWarrantyEnd)<0  ";
		}
		else
		{
			text = text + " and datediff(d,dateadd(dd," + this.ddlType.SelectedValue + ",getdate()),RepairWarrantyEnd)<=0 and datediff(d,getdate(),RepairWarrantyEnd)>0";
		}
