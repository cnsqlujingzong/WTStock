...
public partial class Branch_System_RightAdm : Page, IRequiresSessionState
{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvdata.DataSource = DALCommon.GetList_HL(1, "[Right]", "", this.pageSize, this.jsPager.CurrentPageIndex, text, fldSort, out recordCount);
	...
	{
		string text = " DeptID=" + (string)this.Session["Session_wtBranchID"] + " and bDefault=0 ";
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int num = dALRight.Delete(int.Parse((string)this.Session["Session_wtBranchID"]), iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		DataTable dt = DALCommon.GetDataList("[Right]", this.hfTbField.Value, strCondition).Tables[0];
		...
		string empty = string.Empty;
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "[right]", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
