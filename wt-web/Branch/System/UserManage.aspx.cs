...
public partial class Branch_System_UserManage : Page, IRequiresSessionState
{
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int delAdm = dALUserManage.GetDelAdm(int.Parse(this.Session["Session_wtBranchID"].ToString()), int.Parse(this.hfRecID.Value));
		if (delAdm == 0)
		{
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		string empty = string.Empty;
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "用户管理", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
			this.SysInfo("window.alert('" + empty + "');");
