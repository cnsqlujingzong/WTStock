...
public partial class Branch_Tool_LayoutUserAdd : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			OtherFunction.BindStaff(this.ddlUser, " DeptID=" + (string)this.Session["Session_wtBranchID"]);
			DataTable dataSource = DALCommon.GetDataList("LayoutClass", "[ID],[_Name]", " DeptID=" + (string)this.Session["Session_wtBranchID"]).Tables[0];
	...
	protected void btnSave_Click(object sender, EventArgs e)
	{
		...
		string str = "";
		int num = dALSys.AddLayoutUser(int.Parse((string)this.Session["Session_wtBranchID"]), int.Parse(this.ddlRight.SelectedValue), int.Parse(this.ddlUser.SelectedValue), out str);
		if (num == 0)
		{
