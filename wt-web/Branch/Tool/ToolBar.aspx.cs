...
public partial class Branch_Tool_ToolBar : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			this.gvdata.DataSource = DALCommon.GetDataList("ToolBar", "", " DeptID=" + (string)this.Session["Session_wtBranchID"] + " and iOperator is null order by Array asc");
			this.gvdata.DataBind();
			int num = int.Parse((string)this.Session["Session_wtUserBID"]);
			DataTable dataTable = DALCommon.GetDataList("ToolBar", "", string.Concat(new object[]
			{
				" DeptID=",
	...
	{
		string str = "";
		int num = DALCommon.DeteleData("ToolBar", " DeptID=" + (string)this.Session["Session_wtBranchID"] + " and iOperator=" + (string)this.Session["Session_wtUserBID"], out str);
		if (num == 0)
		{
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		for (int i = 0; i < gridViewSource.Rows.Count; i++)
		{
			...
			{
				string text = "DeptID";
				string text2 = (string)this.Session["Session_wtBranchID"];
				text += ",iOperator";
				text2 = text2 + "," + (string)this.Session["Session_wtUserBID"];
				text += ",FieldValue";
