...
public partial class Headquarters_Basic_InputStockList : Page, IRequiresSessionState
{
	private int iflag;
	...
	protected void ddlTableName_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		{
			if (this.ddl3.Items[j].Text == "管理员2")
			{
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			...
			{
				text2 = FunLibrary.ChkInput(dataTable.Rows[i][this.ddl2.SelectedItem.Text].ToString());
			}
			if (this.ddl3.SelectedItem.Text != "")
			{
				text3 = FunLibrary.ChkInput(dataTable.Rows[i][this.ddl3.SelectedItem.Text].ToString());
			}
			if (this.ddl4.SelectedItem.Text != "")
			{
				text4 = FunLibrary.ChkInput(dataTable.Rows[i][this.ddl4.SelectedItem.Text].ToString());
			}
			if (this.ddl5.SelectedItem.Text != "")
			{
				text5 = FunLibrary.ChkInput(dataTable.Rows[i][this.ddl5.SelectedItem.Text].ToString());
			}
			if (this.ddl6.SelectedItem.Text != "")
			{
				text6 = FunLibrary.ChkInput(dataTable.Rows[i][this.ddl6.SelectedItem.Text].ToString());
			}
			if (text != "" || text2 != "" || text3 != "" || text4 != "" || text5 != "" || text6 != "")
			{
				string text7;
				int num3 = dALStockList.Add_Stock(text, text2, text3, text4, text5, text6, depetid, out text7);
				if (num3 == 0)
				{
					num++;
				}
		...
		{
			this.SysInfo("window.alert(\"导入全部失败！仓库信息已存在\");");
		}
		else
		{
			this.SysInfo(string.Concat(new string[]
			{
				...
				"条记录，失败",
				num2.ToString(),
				"条记录，失败原因：仓库信息已存在\");parent.CloseDialog('1');"
			}));
		}
	}
