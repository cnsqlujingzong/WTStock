...
public partial class Headquarters_Basic_TroubleClassAdm : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "jc_r15"))
				{
	...
	{
		int num = 0;
		DataTable dt = DALCommon.GetList_HL(0, "TroubleClass", "", 0, 0, "", " Array Asc", out num).Tables[0];
	...
	protected void btnAdds_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			dataClassInfo.Father = new int?(-1);
			int array = 0;
			int.TryParse(this.tbArray.Text, out array);
			dataClassInfo.Array = array;
			string str;
			int num = dALDataClass.Add("TroubleClass", dataClassInfo, out str);
			if (num == 0)
			{
	...
	protected void btnMod_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			string str;
			int num = dALDataClass.Update(0, "TroubleClass", dataClassInfo, chkfld, 0, out str);
			if (num == 0)
			{
				...
				this.SetEnable();
				this.FillData();
				this.TreeView1.FindNode(this.hfValuePath.Value).Select();
			}
			else if (num == -1)
			{
				this.SysInfo("window.alert('" + str + "');$('tbName').select();");
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int num = dALDataClass.Delete("TroubleClass", iD, out empty);
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
			this.FillData();
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			DataClassInfo dataClassInfo = new DataClassInfo();
			dataClassInfo._Name = FunLibrary.ChkInput(this.tbName.Text);
			dataClassInfo.Father = new int?(int.Parse(this.hfRecID.Value));
			int array = 0;
			int.TryParse(this.tbArray.Text, out array);
			dataClassInfo.Array = array;
			string str;
	...
	protected void TreeView1_SelectedNodeChanged(object sender, EventArgs e)
	{
		...
		DALDataClass dALDataClass = new DALDataClass();
		DataClassInfo model = dALDataClass.GetModel("TroubleClass", int.Parse(this.hfRecID.Value));
		if (model != null)
		{
