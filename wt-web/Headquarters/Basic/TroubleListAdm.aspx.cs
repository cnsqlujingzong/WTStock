...
public partial class Headquarters_Basic_TroubleListAdm : Page, IRequiresSessionState
{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvtrouble.DataSource = DALCommon.GetList_HL(1, "jc_trouble", "", this.pageSize, this.jsPager.CurrentPageIndex, text, fldSort, out recordCount);
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int num = DALCommon.DeteleData(0, 0, iTbid, out empty);
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
		}
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		DataTable dt = DALCommon.GetDataList("jc_trouble", this.hfTbField.Value, strCondition).Tables[0];
		...
		string empty = string.Empty;
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "故障列表", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
			this.SysInfo("window.alert('" + empty + "');");
	...
	{
		int num = 0;
		this.GridView2.DataSource = DALCommon.GetList_HL(0, "TakeSteps", "", 0, 0, " TroubleID=" + this.hfRecID.Value, " ID desc ", out num);
