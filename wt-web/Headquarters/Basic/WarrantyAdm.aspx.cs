...
public partial class Headquarters_Basic_WarrantyAdm : Page, IRequiresSessionState
{
	...
	{
		int recordCount = 0;
		this.GridView1.DataSource = DALCommon.GetList_HL(1, "Warranty", "[ID],[_Name],bCall=case when bCall=1 then '√' else '' end,bStopUse=case when bStopUse=1 then '√' else '' end,Array", this.pageSize, this.jsPager.CurrentPageIndex, "", " Array Asc ", out recordCount);
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			DALWarranty dALWarranty = new DALWarranty();
			warrantyInfo._Name = FunLibrary.ChkInput(this.tbName.Text);
			warrantyInfo.pyCode = FunLibrary.CVT(FunLibrary.ChkInput(this.tbName.Text));
			warrantyInfo.bCall = this.cbCall.Checked;
			warrantyInfo.bStopUse = this.cbStopUse.Checked;
			int array = 0;
			int.TryParse(this.tbArray.Text, out array);
			warrantyInfo.Array = array;
			if (this.hfRecID.Value == "-1")
			{
				this.hfTemp.Value = warrantyInfo._Name;
				string str;
				int num2;
				int num = dALWarranty.Add(warrantyInfo, out str, out num2);
				if (num == 0)
				{
			...
			{
				warrantyInfo.ID = int.Parse(this.hfRecID.Value);
				string text = string.Empty;
				if (warrantyInfo._Name != this.hfTemp.Value)
				{
					text = text + " _Name='" + warrantyInfo._Name + "'";
				}
				string str;
				int num = dALWarranty.Update(warrantyInfo, text, out str);
				if (num == 0)
				{
					this.hfRecID.Value = warrantyInfo.ID.ToString();
				}
				else
				{
					if (num == -1)
	...
	protected void btnMod_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			string str2;
			int num = dALWarranty.Update(warrantyInfo, text, out str2);
			if (num == 0)
			{
				this.hfRecID.Value = warrantyInfo.ID.ToString();
				this.FillData();
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		int num = dALBaseInfo.Delete("Warranty", iD, out empty);
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
		}
	...
	protected void btnShow_Click(object sender, EventArgs e)
	{
		...
		{
			DALWarranty dALWarranty = new DALWarranty();
			WarrantyInfo model = dALWarranty.GetModel(int.Parse(this.hfRecID.Value));
			this.tbName.Text = model._Name;
			this.hfTemp.Value = model._Name;
			this.cbCall.Checked = model.bCall;
			this.cbStopUse.Checked = model.bStopUse;
