<%@ page language="C#"  CodeFile="sltsup.aspx.cs"     autoeventwireup="true" inherits="Headquarters_Basic_SltSup" theme="Themes" enableEventValidation="false" %>
...
function ChkSup()
{
    ...
    if(<%=Str_D %>=="1")
    {
        try
        {
            parent.<%=Str_Fid %>.$("tbSupName").value=$("hfName").value;
            parent.<%=Str_Fid %>.$("hfSupID").value=$("hfRecID").value;
            parent.<%=Str_Fid %>.$("btnSupInfo").click();
        }catch(e){}
        
    }else
    {
        try{
            var ddlgradelevel=parent.<%=Str_Fid %>.$("ddlProvider");
