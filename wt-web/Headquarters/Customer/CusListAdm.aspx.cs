...
public partial class Headquarters_Customer_CusListAdm : Page, IRequiresSessionState
{
	...
	protected void LoadClass()
	{
		...
		Headquarters_Customer_CusListAdm.BindTreeNode(treeNode, dt, treeNode.Value);
	...
	public static void BindTreeNode(TreeNode node, DataTable dt, string parentid)
	{
		...
		for (int i = 0; i < array2.Length; i++)
		{
			...
			Headquarters_Customer_CusListAdm.BindTreeNode(treeNode, dt, dataRow["ID"].ToString());
		}
	}
	...
	protected void LoadField()
	{
		...
		if (dataTable.Rows.Count > 0)
		{
			...
			templateField.ItemTemplate = new MyTemplatexcus("", DataControlRowType.DataRow);
	...
	protected string strParm()
	{
		...
		if (this.hfParm.Value == "-1")
		{
			...
			text2 = FunLibrary.SqlSeltC(text2);
	...
	protected void btnDelCont_Click(object sender, EventArgs e)
	{
		...
		int num = dALMaintenanceContract.Delete(1, iTbid, iOperator, out empty);
		if (num == 0)
		{
			this.hfRecID2.Value = "-1";
		}
