...
public partial class Headquarters_Customer_CusMerge : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			DataSet list = DALCommon.GetList("CustomerList", "ID,_Name,LinkMan,Tel,Adr,Remark", string.Format("ID in({0})", this.ids));
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		if (num <= 0)
		{
			this.SysInfo("alert('请选择要合并为哪个客户');");
		}
		else
		{
			DALCustomerList dALCustomerList = new DALCustomerList();
			bool flag = dALCustomerList.CusMerge(num, this.ids, this.tbCusName.Text.Trim(), this.tbLinkMan.Text.Trim(), this.tbLinkTel.Text.Trim(), this.tbAdr.Text.Trim(), this.tbRemark.Text.Trim());
			if (flag)
			{
