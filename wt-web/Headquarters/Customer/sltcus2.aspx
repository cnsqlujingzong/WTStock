<%@ page language="C#"  CodeFile="sltcus2.aspx.cs"     autoeventwireup="true" inherits="Headquarters_Customer_SltCus2" theme="Themes" enableEventValidation="false" %>
...
function ChkIDCus(strname,strcusno)
{
...
function ChkCus()
{
    ...
    try{
    parent.<%=Str_Fid %>.$("hfCusID2").value=$("hfRecID").value;
    parent.<%=Str_Fid %>.$("tbCusName2").value=$("hfCusName").value;
    if("<%=Str_F %>"=="2")
      {
        parent.<%=Str_Fid %>.$("tbCusNO2").value=$("hfCusNO").value;
        parent.<%=Str_Fid %>.$("btnCusInfo2").click();
      }
    if("<%=Str_F %>"=="3")
      {
        parent.<%=Str_Fid %>.$("btnCusInfo2").click();
