...
public partial class Headquarters_Financial_IncomeAdd : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "zk_r1"))
				{
	...
	protected void gvdata_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.DataRow)
		{
			...
			else if (e.Row.Cells[2].Text == "完工结算")
			{
				e.Row.Cells[3].Text = string.Concat(new string[]
				{
			...
			else if (e.Row.Cells[2].Text == "租金结算")
			{
				e.Row.Cells[3].Text = string.Concat(new string[]
				{
					"<a href=\"#\" onclick=\"Lease('",
					e.Row.Cells[3].Text,
					"');\">",
					e.Row.Cells[3].Text,
					"</a>"
			...
			else if (e.Row.Cells[2].Text == "采购退货")
			{
				e.Row.Cells[3].Text = string.Concat(new string[]
				{
	...
	protected void btnId_Click(object sender, EventArgs e)
	{
		...
		if (this.hfCusID.Value != "")
		{
			...
			DataTable dataTable = DALCommon.GetDataList("zk_arrearage", "Balance", "DeptID=1 and CusType=" + this.hfType.Value + " and CustomerID=" + text).Tables[0];
			if (dataTable.Rows.Count > 0)
			{
				this.tbBalance.Text = dataTable.Rows[0]["Balance"].ToString();
			}
			DataTable gridViewSource = this.GridViewSource;
			DataTable dataTable2 = DALCommon.GetDataList("zk_cusarrearage", "", " DeptID=1 and Status=1 and NotChargeAmount>0 and Type='应收款' and CustomerID=" + text + " and CusType=" + this.hfType.Value).Tables[0];
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num != num2)
			{
				this.SysInfo("window.alert('操作失败！本次应收款与单据明细不符，请修改！');");
			}
			else if (num2 > num)
			{
				this.SysInfo("window.alert('操作失败!应收款不能大于本次结算总金额，请修改！');");
			}
			else
			{
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					this.SysInfo("window.alert('操作成功！应收款已保存！');");
					this.ClearText();
	...
	protected int BillAdd(out int iTbid, out string strMsg)
	{
		...
		incomeExpensesInfo.RecType = "应收款";
		...
		incomeExpensesInfo.RecMoney = decimal.Parse(this.tbRecMoney.Text);
		incomeExpensesInfo.PreMoney = decimal.Parse(this.tbPreMoney.Text);
		incomeExpensesInfo.RealRecMoney = decimal.Parse(this.tbRealMoney.Text);
		...
		DALIncomeExpenses dALIncomeExpenses = new DALIncomeExpenses();
		int result = dALIncomeExpenses.Add(incomeExpensesInfo, 1, out iTbid, out strMsg);
		this.hfPrintID.Value = iTbid.ToString();
		return result;
	}
