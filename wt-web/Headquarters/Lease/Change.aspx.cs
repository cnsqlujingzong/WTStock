...
public partial class Headquarters_Lease_Change : Page, IRequiresSessionState
{

	private int id;

	private static decimal inputcount = 1m;
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (dataTable.Rows.Count > 0)
			{
				...
				{
					decimal.TryParse(dataTable.Rows[0]["iCount"].ToString(), out Headquarters_Lease_Change.inputcount);
				}
				...
				this.hfGoodsNO.Value = (this.tbGoodsNO.Text = (this.tbInGoodsNO.Text = dataTable.Rows[0]["GoodsNO"].ToString()));
				...
				if (Headquarters_Lease_Change.inputcount == 0m)
				{
					Headquarters_Lease_Change.inputcount = 1m;
				}
				this.tbInCount.Text = Headquarters_Lease_Change.inputcount.ToString();
				if (dataTable.Rows[0]["Type"].ToString() == "非抄表类租赁" || dataTable.Rows[0]["Type"].ToString() == "全保")
				{
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		decimal num;
		if (this.hfType.Value == "抄表类租赁")
		{
			num = Headquarters_Lease_Change.inputcount;
		}
		else
		{
			decimal.TryParse(this.tbOutCount.Text, out num);
			if (num == 0m)
			{
				num = 1m;
			}
		}
		int.TryParse(this.tbInCount.Text, out inCount);
		decimal.TryParse(this.tbOutPrice.Text, out doutprice);
		string str = "";
		DALLease dALLease = new DALLease();
		int num2 = dALLease.ChkZLHJ(doutprice, inCount, 0, this.id, int.Parse(this.ddlBrand.SelectedValue), int.Parse(this.ddlClass.SelectedValue), int.Parse(this.ddlModel.SelectedValue), FunLibrary.ChkInput(this.tbSN.Text), iInStockid, dPrice, iGoodsid, int.Parse(this.ddlStock.SelectedValue), int.Parse(this.Session["Session_wtUserID"].ToString()), FunLibrary.ChkInput(this.tbDeviceNO.Text), FunLibrary.ChkInput(this.tbQty.Text), num, out str);
		if (num2 == 0)
		{
	...
	private void insertCount(string strs, int count)
	{
		...
		if (flag)
		{
			dALLease.insertValue(num, leaseDeviceID - count, lastDeviceID, int.Parse(this.Session["Session_wtUserID"].ToString()), qtyType, qty);
		}
	}
