...
public partial class Headquarters_Lease_Charge : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			DataTable dataTable = DALCommon.GetDataList("Lease", "Rent,ChargeCycle,Type", " [ID]=" + this.id).Tables[0];
			if (dataTable.Rows.Count > 0)
			{
				...
				a = dataTable.Rows[0]["Type"].ToString();
				if (a == "2")
				{
					this.btnCalc.Visible = true;
				}
				else
				{
				...
				for (int j = 0; j < dataTable2.Rows.Count; j++)
				{
					...
					if (dataTable4.Rows.Count > 0)
					{
						for (int i = 0; i < dataTable4.Rows.Count; i++)
						{
							...
							if (count > 0)
							{
								...
								decimal d9 = num3 * int.Parse(dataTable4.Rows[i]["Rated"].ToString());
								d2 += d9;
					...
					if (dataTable6.Rows.Count > 0)
					{
						for (int i = 0; i < dataTable6.Rows.Count; i++)
						{
							...
							decimal num17 = num3 * decimal.Parse(dataTable6.Rows[i]["Rated"].ToString());
							...
							{
								int num18 = 0;
								int.TryParse(dataRow[3].ToString(), out num18);
								decimal num19 = 0m;
								int num20 = 0;
								int.TryParse(dataRow[4].ToString(), out num20);
								num18 -= num20;
								int num21 = 0;
								int num22 = 0;
								...
								if (dataTable8.Rows.Count > 0)
								{
									for (int k = 0; k < dataTable8.Rows.Count; k++)
									{
										if (num18 > 0)
										{
											int.TryParse(dataTable8.Rows[k]["StartQty"].ToString(), out num21);
											int.TryParse(dataTable8.Rows[k]["EndQty"].ToString(), out num22);
											int num23 = num22 - num21 + 1;
											if (num18 > num23)
											{
												num19 += num23 * decimal.Parse(dataTable8.Rows[k]["Price"].ToString());
											}
											else
											{
												num19 += num18 * decimal.Parse(dataTable8.Rows[k]["Price"].ToString());
											}
											num18 -= num23;
										}
									}
									if (num18 > 0)
									{
										num19 += num18 * decimal.Parse(dataTable6.Rows[i]["SuperZhangFee"].ToString());
									}
								}
								dataRow[6] = num19;
							}
							...
							if (num15 == 4)
							{
								...
								{
									DateTime d10 = DateTime.Parse(dataTable6.Rows[i]["ChargeDate"].ToString());
									DateTime d11 = DateTime.Parse(this.tbEndDate.Text);
									decimal num24 = decimal.Parse((d11 - d10).Days.ToString()) / d;
									num24 = decimal.Round(num24, 0);
									if (num24 == 3m)
									{
										string text = d11.AddMonths(-3).ToString("yyyy-MM-dd");
							...
							if (num15 == 7)
							{
								if (num8 > 0m)
								{
									int num25 = int.Parse(dataRow["BQty"].ToString()) - int.Parse(dataRow["SQty"].ToString()) - int.Parse(dataRow["Loss"].ToString());
									int num26 = int.Parse(dataRow["Rated"].ToString()) - num25;
									if (num26 < 0)
									{
										decimal d12 = num8 / decimal.Parse(dataRow[5].ToString());
										decimal num27 = -num26;
										if (d12 > num27)
										{
											dataRow[3] = num25;
											dataRow[6] = 0;
											num8 -= num27 * decimal.Parse(dataRow["ZhangFee"].ToString());
										}
										else
										{
											dataRow[3] = num25;
											dataRow[6] = (num27 * decimal.Parse(dataRow["ZhangFee"].ToString()) - num8).ToString("f2");
											num8 = 0m;
										}
									}
								}
							...
							if (num15 == 8)
							{
								...
								if (dataTable9.Rows.Count > 0)
								{
									decimal d13 = decimal.Parse(dataRow[2].ToString());
									decimal d14 = decimal.Parse(dataTable9.Rows[0]["Qty"].ToString());
									decimal d15 = d13 - d14;
									if (d15 > decimal.Parse(dataRow[4].ToString()))
									{
										dataRow[4] = 0;
										dataRow[3] = decimal.Parse(dataRow[1].ToString()) - decimal.Parse(dataRow[2].ToString()) - decimal.Parse(dataRow[11].ToString());
									}
				...
				decimal num28 = 0m;
				decimal.TryParse(this.tbARent.Text, out num28);
				...
				decimal num29 = num28 + num4;
				this.tbRec.Text = num29.ToString("#0.00");
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		int num4 = 0;
		decimal dRec = 0m;
		decimal.TryParse(this.tbRec.Text, out dRec);
		...
		int operatorID = 0;
		int.TryParse((string)this.Session["Session_wtUserID"], out operatorID);
		...
		int num6;
		int num5 = dALLeaseCharge.Add(leaseChargeInfo, out num6);
		if (num5 == 0)
		{
			...
			for (int j = 0; j < dataTable2.Rows.Count; j++)
			{
				...
				if (dataTable4.Rows.Count > 0)
				{
					for (int i = 0; i < dataTable4.Rows.Count; i++)
					{
						...
						decimal d9 = num2 * int.Parse(dataTable4.Rows[i]["Rated"].ToString());
						d2 += d9;
				...
				if (dataTable7.Rows.Count > 0)
				{
					for (int i = 0; i < dataTable7.Rows.Count; i++)
					{
						...
						if (num14 == 4)
						{
							if (this.tbEndDate.Text != "")
							{
								DateTime d10 = DateTime.Parse(dataTable7.Rows[i]["ChargeDate"].ToString());
								DateTime d11 = DateTime.Parse(this.tbEndDate.Text);
								decimal num22 = decimal.Parse((d11 - d10).Days.ToString()) / d;
								num22 = decimal.Round(num22, 0);
								if (num22 == 3m)
								{
									string text3 = d11.AddMonths(-3).ToString("yyyy-MM-dd");
									dataRow[4] = decimal.Parse(dataRow[9].ToString()) * 3m;
									...
									if (dataTable11.Rows.Count > 0)
									{
                                        ...
                                        decimal d1 = decimal.Parse(dataRow[1].ToString());
                                        ++d1;
										dataRow[3] = d1 - decimal.Parse(dataRow[2].ToString()) - decimal.Parse(dataRow[11].ToString());
									}
									dataRow[6] = (decimal.Parse(dataRow[3].ToString()) - decimal.Parse(dataRow[4].ToString())) * decimal.Parse(dataRow[5].ToString());
								}
								else
						...
						if (num14 == 5)
						{
							if (this.tbEndDate.Text != "")
							{
								...
								if (num22 == 3m)
								{
									...
									{
										dataRow[2] = decimal.Parse(dataTable11.Rows[0]["Qty"].ToString());
                                        decimal d_1 = (decimal.Parse(dataRow[1].ToString()) - decimal.Parse(dataRow[2].ToString()) - decimal.Parse(dataRow[11].ToString()));
                                        ++d_1;
                                        dataRow[3] = d_1;
									}
									dataRow[6] = (decimal.Parse(dataRow[3].ToString()) - decimal.Parse(dataRow[4].ToString())) * decimal.Parse(dataRow[5].ToString());
						...
						if (num14 == 7)
						{
							if (num8 > 0m)
							{
								int num23 = int.Parse(dataRow["BQty"].ToString()) - int.Parse(dataRow["SQty"].ToString()) - int.Parse(dataRow["Loss"].ToString() + 1);
						...
						if (num14 == 8)
						{
							...
							{
								decimal d13 = decimal.Parse(dataRow[2].ToString());
								decimal d14 = decimal.Parse(dataTable11.Rows[0]["Qty"].ToString());
								decimal d15 = d13 - d14;
								if (d15 > decimal.Parse(dataRow[4].ToString()))
								{
									dataRow[4] = 0;
	...
	protected void btnCalc_Click(object sender, EventArgs e)
	{
		...
		DataTable dataTable = DALCommon.GetDataList("Lease", "Rent,ChargeCycle,ReadingDate", " [ID]=" + this.id).Tables[0];
		if (dataTable.Rows.Count > 0)
		{
			decimal.TryParse(dataTable.Rows[0]["Rent"].ToString(), out d);
			decimal.TryParse(dataTable.Rows[0]["ChargeCycle"].ToString(), out num);
			DateTime.TryParse(dataTable.Rows[0]["ReadingDate"].ToString(), out now);
		}
		DataTable dataTable2 = DALCommon.GetDataList("LeaseCharge", "top 1 EndDate", "BillID=" + this.id + " order by ID desc").Tables[0];
		if (dataTable2.Rows.Count > 0)
		{
			DateTime.TryParse(dataTable2.Rows[0]["EndDate"].ToString(), out now);
		}
		this.tbStartDate.Text = now.ToShortDateString();
		this.tbEndDate.Text = DateTime.Now.ToShortDateString();
		TimeSpan timeSpan = DateTime.Now - now;
		if (num == 0m)
		{
			num = 1m;
		}
		int value = (int)(timeSpan.Days / num);
		this.tbCycle.Text = (this.tbCycled.Text = value.ToString());
		this.tbRent.Text = d.ToString();
		this.tbARent.Text = (this.tbAARent.Text = (this.tbRec.Text = (d * value).ToString()));
	}
}
