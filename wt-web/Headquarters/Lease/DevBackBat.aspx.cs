...
public partial class Headquarters_Lease_DevBackBat : Page, IRequiresSessionState
{
	...
	private DataTable GridViewSource
	{
		get
		{
			if (this.ViewState["List"] == null)
			{
				...
				dataTable.Columns.Add(new DataColumn("InCount", typeof(decimal)));
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			DataSet dataList = DALCommon.GetDataList("zl_leasedevice", "ID,GoodsNO,StockID,iCount,OutPrice,ProductSN1,DeviceNO", string.Format("BillID in(select ID from Lease where  status=1 and customerid=(select customerID from zl_leasedevice where id={0})) and DeptID=1 and DevStatus='正常'", this.id));
			if (dataList.Tables[0].Rows.Count > 0)
			{
				...
				for (int i = 0; i < dataList.Tables[0].Rows.Count; i++)
				{
					...
					dataRow["InCount"] = dataList.Tables[0].Rows[i]["iCount"].ToString();
	...
	protected void gvdata_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.DataRow)
		{
			...
			{
				this.dsstock = DALCommon.GetDataList("StockList", "ID,_Name", "DeptID=1");
			}
	...
	protected void CollectData()
	{
		...
		for (int i = 0; i < this.gvdata.Rows.Count; i++)
		{
			...
			gridViewSource.Rows[i]["InCount"] = textBox.Text;
		}
	}
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		for (int i = 0; i < gridViewSource.Rows.Count; i++)
		{
			...
			int.TryParse(gridViewSource.Rows[i]["ID"].ToString(), out iTbid);
			...
			int.TryParse(gridViewSource.Rows[i]["InCount"].ToString(), out num3);
			if (num3 == 0)
			{
				num3 = 1;
			}
			int num4 = dALLease.ChkZLTJ(iTbid, iInStockid, dPrice, int.Parse(this.Session["Session_wtUserID"].ToString()), num3, out empty);
			if (num4 == 0)
			{
				num++;
			}
	...
	protected void btnSltIDs_Click(object sender, EventArgs e)
	{
		...
		if (text != "")
		{
			DataSet dataList = DALCommon.GetDataList("zl_leasedevice", "ID,GoodsNO,StockID,OutPrice,DeviceNO,ProductSN1,iCount", string.Format("ID in({0}) and DevStatus='正常' and DeptID=1", text));
			DataTable gridViewSource = this.GridViewSource;
			for (int i = 0; i < dataList.Tables[0].Rows.Count; i++)
			{
