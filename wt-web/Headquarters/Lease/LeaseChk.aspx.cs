...
public partial class Headquarters_Lease_LeaseChk : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "zl_r21"))
				{
				...
				{
					Headquarters_Lease_LeaseChk.bNeedStockCHK = true;
				}
			}
	...
	{
		int recordCount = 0;
		string text = (this.hfParm.Value == "-1") ? this.strParm() : (this.hfParm.Value + " and DeptID=1 and Status=0");
	...
	{
		string text = " DeptID=1 and Status=0 ";
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				int iflag;
				if (Headquarters_Lease_LeaseChk.bNeedStockCHK)
				{
					iflag = 1;
				}
				else
				...
				int num4 = dALLease.LeaseChk(num3, iflag, iOperator, out empty);
				if (num4 == 0)
				{
					num2++;
				}
	...
	{
		e.Row.Cells[0].Visible = false;
		if (Headquarters_Lease_LeaseChk.leaseType != 2)
		{
	...
	{
		Headquarters_Lease_LeaseChk.leaseType = new DALLease().getLeaseType(int.Parse(this.hfRecID.Value));
