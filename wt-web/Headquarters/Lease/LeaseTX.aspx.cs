...
public partial class Headquarters_Lease_LeaseTX : Page, IRequiresSessionState
{
	...
	{
		string text = " DeptID=1 and curStatus='正常' ";
		if (this.ddlType.SelectedValue == "0")
		{
			text += " and datediff(d,getdate(),EndDate)<0 ";
		}
		else
		{
			text = text + " and datediff(d,dateadd(dd," + this.ddlType.SelectedValue + ",getdate()),EndDate)<=0 ";
		}
