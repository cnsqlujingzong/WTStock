...
public partial class Headquarters_Lease_MeterReading : Page, IRequiresSessionState
{
	...
	{
		int recordCount = 0;
		string str = FunLibrary.ChkInput(this.ddlBranch.SelectedValue);
		string text = (this.hfParm.Value == "-1") ? this.strParm() : (this.hfParm.Value + " and DeptID=" + str + " and Status=1");
			...
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 1)
				{
					...
					if (dataField != "" && dataField != "ID")
					{
						...
						{
							HiddenField expr_354 = this.hfTbTitle;
							expr_354.Value = expr_354.Value + "," + text2;
						}
						...
						{
							HiddenField expr_3A6 = this.hfTbField;
							expr_3A6.Value = expr_3A6.Value + "," + dataField;
						}
					}
				}
			}
	...
	{
		string str = FunLibrary.ChkInput(this.ddlBranch.SelectedValue);
		string text = " DeptID=" + str + " and Status=1 ";
		if (this.iFlag == 2)
		{
	...
	protected void btnChkU_Click(object sender, EventArgs e)
	{
		...
		int num = dALLease.LeaseChkU(1, iTbid, iOperator, out empty);
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
		string text = "";
		if (num != 0)
		{
	...
	{
		e.Row.Cells[0].Visible = false;
		if (Headquarters_Lease_MeterReading.leaseType != 2)
		{
	...
	{
		Headquarters_Lease_MeterReading.leaseType = new DALLease().getLeaseType(int.Parse(this.hfRecID.Value));
	...
	protected void btnSchDept_Click(object sender, EventArgs e)
	{
		this.tbCon.Text = "";
		this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
	}
