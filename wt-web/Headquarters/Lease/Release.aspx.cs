...
public partial class Headquarters_Lease_Release : Page, IRequiresSessionState
{
	...
	{
		int recordCount = 0;
		string text = (this.hfParm.Value == "-1") ? this.strParm() : (this.hfParm.Value + " and DeptID=1 and Status=1");
		...
		if (this.gvdata.Rows.Count > 0)
		{
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 1)
				{
					...
					{
						HiddenField expr_303 = this.hfTbTitle;
						expr_303.Value = expr_303.Value + "," + text2;
					}
					...
					{
						HiddenField expr_353 = this.hfTbField;
						expr_353.Value = expr_353.Value + "," + dataField;
					}
				}
			}
		}
	...
	{
		string text = " DeptID=1 and Status=1 ";
	...
	{
		e.Row.Cells[0].Visible = false;
		if (Headquarters_Lease_Release.leaseType != 2)
		{
	...
	{
		Headquarters_Lease_Release.leaseType = new DALLease().getLeaseType(int.Parse(this.hfRecID.Value));
