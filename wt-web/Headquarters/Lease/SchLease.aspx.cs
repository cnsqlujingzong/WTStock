...
public partial class Headquarters_Lease_SchLease : Page, IRequiresSessionState
{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		{
			for (int i = 1; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		if (num2 > 0)
		{
			num = dALLease.ChkDel(num2, int.Parse((string)this.Session["Session_wtUserID"]), out empty);
		}
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
	...
	{
		e.Row.Cells[0].Visible = false;
		if (Headquarters_Lease_SchLease.leaseType != 2)
		{
	...
	{
		Headquarters_Lease_SchLease.leaseType = new DALLease().getLeaseType(int.Parse(this.hfRecID.Value));
