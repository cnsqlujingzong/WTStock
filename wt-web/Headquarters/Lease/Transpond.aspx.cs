...
public partial class Headquarters_Lease_Transpond : Page, IRequiresSessionState
{
	...
	protected void btnAdd_Click(object sender, EventArgs e)
	{
		...
		if (this.id > 0)
		{
			int num = dALLease.Transpond(this.id, int.Parse(this.ddlBranch.SelectedValue), int.Parse(this.Session["Session_wtUserID"].ToString()));
			if (num > 0)
			{
		...
		else
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				int num = dALLease.Transpond(num4, int.Parse(this.ddlBranch.SelectedValue), int.Parse(this.Session["Session_wtUserID"].ToString()));
				if (num > 0)
				{
					num2++;
				}
