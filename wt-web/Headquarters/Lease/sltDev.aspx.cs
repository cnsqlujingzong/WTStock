...
public partial class Headquarters_Lease_sltDev : Page, IRequiresSessionState
{
	private int id;
	...
	protected void BindData(string strWhere)
	{
		...
		string text2 = strWhere + string.Format("BillID in(select ID from Lease where  status=1 and customerid=(select customerID from zl_leasedevice where id={0})) and DevStatus='正常' and DeptID=1 ", this.id);
		if (!text.Equals(""))
		{
