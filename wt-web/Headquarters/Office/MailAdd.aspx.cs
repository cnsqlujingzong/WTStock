...
public partial class Headquarters_Office_MailAdd : Page, IRequiresSessionState
{
	private int id;
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (base.Request.QueryString["id"] == null)
			{
				...
				stringBuilder.Append('\r'.ToString());
				stringBuilder.Append("  发件人：" + (string)this.Session["Session_wtUser"] + "  时间：" + DateTime.Now.ToString());
				...
				if (this.id != 0)
				{
					...
					if (model != null)
					{
						...
						if (text == "0")
						{
							...
							stringBuilder2.Append("==============================");
							stringBuilder2.Append('\r'.ToString());
							stringBuilder2.Append("  发件人：" + (string)this.Session["Session_wtUser"] + "  时间：" + DateTime.Now.ToString());
							stringBuilder2.Append('\r'.ToString());
							stringBuilder2.Append('\r'.ToString());
	...
	protected void MailSend(int flag)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (iD > 0)
			{
				...
				oA_EmailInfo.SndID = new int?(int.Parse((string)this.Session["Session_wtUserID"]));
