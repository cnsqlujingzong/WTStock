...
public partial class Headquarters_Office_MailAdm : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "bg_r8"))
				{
	...
	protected void LoadClass()
	{
		...
		string str = dALOA_Mail.TCount(" RcvID='" + int.Parse((string)this.Session["Session_wtUserID"]) + "' and bRead=0 and bSndFlag=1 and bRcvFlag=0 ");
		this.tv.Nodes.Clear();
		TreeNode treeNode = new TreeNode();
		treeNode.Text = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + (string)this.Session["Session_wtUser"];
		treeNode.Value = "-1";
			...
			if (!(value == "-1") && !(value == "0"))
			{
				if (!(value == "1"))
				{
					if (!(value == "2"))
					{
						if (value == "3")
						{
							text = text + " and RcvID='" + (string)this.Session["Session_wtUserID"] + "' and bRcvFlag=1 ";
						}
					}
					else
					{
						text = text + " and SndID='" + (string)this.Session["Session_wtUserID"] + "'and bSndFlag=1 ";
					}
				}
				else
				{
					text = text + " and SndID='" + (string)this.Session["Session_wtUserID"] + "' and bSndFlag=0 ";
				}
			}
			else
			{
				text = text + " and RcvID='" + (string)this.Session["Session_wtUserID"] + "' and bRcvFlag=0 and bSndFlag=1 ";
			}
		}
