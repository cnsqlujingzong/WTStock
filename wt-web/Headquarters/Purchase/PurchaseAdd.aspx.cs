...
public partial class Headquarters_Purchase_PurchaseAdd : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				{
					Headquarters_Purchase_PurchaseAdd.bCheck = true;
				}
			}
				...
				for (int i = 0; i < dataTable.Rows.Count; i++)
				{
					...
					if (flag)
					{
						...
						else if (d > 0m)
						{
                            ...
                            decimal TaxRate1 = decimal.Parse(dataTable.Rows[i]["TaxRate"].ToString()); ++TaxRate1;
                            decimal TaxRate2 = decimal.Parse(dataTable.Rows[i]["TaxRate"].ToString()); ++TaxRate2;
		...
		if (dataTable.Rows.Count == 1)
		{
			...
			else
			{
				...
				if (num == 0)
				{
					...
					if (Headquarters_Purchase_PurchaseAdd.bCheck)
					{
