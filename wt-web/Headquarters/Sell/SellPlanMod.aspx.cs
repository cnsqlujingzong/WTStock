...
public partial class Headquarters_Sell_SellPlanMod : Page, IRequiresSessionState
{
    ...
    protected int BillUpdate(out string strMsg)
    {
        ...
        SellPlanInfo model = new SellPlanInfo
        {
            ...
            LinkMan = FunLibrary.ChkInput(this.ddl_LinkMan.SelectedItem.Text),
            Tel = FunLibrary.ChkInput(this.tbTel.Text),
            Adr = FunLibrary.ChkInput(this.tbAdr.Text),
        
        BrandName = FunLibrary.ChkInput(tbwangdian.Text),
        BrandTaxRate = decimal.Parse(FunLibrary.ChkInput(ps[1])),
        BrandTaxRateType = FunLibrary.ChkInput(ps[0])
        };
        DataTable gridViewSource = this.GridViewSource;
            ...
            if (table3.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < table3.Rows.Count; i++)
                {
                    ...
                    if (flag2)
                    {
                        ...
                        row[11] = "";
                        row[12] = int.Parse(table3.Rows[i]["StockID"].ToString());
    ...
    protected void CollectData()
    {
        ...
        for (int i = 0; i < this.gvdata.Rows.Count; i++)
        {
            ...
            TextBox box6 = this.gvdata.Rows[i].FindControl("tbhuoqi") as TextBox;
            ...
            gridViewSource.Rows[i]["Huoqi"] = FunLibrary.ChkInput(box6.Text);
