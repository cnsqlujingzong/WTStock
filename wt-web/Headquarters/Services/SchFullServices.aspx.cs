...
public partial class Headquarters_Services_SchFullServices : Page, IRequiresSessionState
{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		{
			text = text + " and (SellerID in(select ID from StaffList where StaffDeptID=(select StaffDeptID from StaffList where ID=" + (string)this.Session["Session_wtUserID"] + ")) or OperatorID is null)";
		}
			...
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 1)
				{
					...
					if (dataField != "ID" && flag)
					{
						...
						{
							HiddenField expr_4D2 = this.hfTbTitle;
							expr_4D2.Value = expr_4D2.Value + "," + text2;
						}
						...
						{
							HiddenField expr_522 = this.hfTbField;
							expr_522.Value = expr_522.Value + "," + dataField;
						}
					}
				}
			}
	...
	protected void btnDel_Click(object sender, EventArgs e)
	{
		...
		if (num2 > 0)
		{
			num = dALServices.ChkDel(num2, int.Parse((string)this.Session["Session_wtUserID"]), out empty);
		}
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
	...
	{
		string strCondition = this.hfSql.Value.Replace("''", "'");
		DALServices dALServices = new DALServices();
		int num = 0;
		DataSet fullServices = dALServices.GetFullServices(strCondition, this.hfOrderName.Value + " " + this.hfOrder.Value, 0, 0, 0, out num);
		DataTable dataTable = fullServices.Tables[0];
		string[] array = this.DealFields(fullServices, false);
		string text = this.hfTbTitle.Value.Trim() + array[0].Trim();
		string text2 = this.hfTbField.Value.Trim() + array[1].Trim();
		string[] tbTitle = text.TrimEnd(new char[]
		{
			','
		}).Split(new char[]
		{
		...
		string[] array2 = text2.TrimEnd(new char[]
		{
			','
		}).Split(new char[]
		{
