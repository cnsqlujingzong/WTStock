...
public partial class Headquarters_Services_ServicesBln : Page, IRequiresSessionState
{
	...
	protected void LoadField()
	{
		...
		DataTable dataTable = dALSys.GetLayoutDetail(1, 1, 11, 0, userID).Tables[0];
		...
		if (dataTable.Rows.Count > 0)
		{
			...
			templateField.ItemTemplate = new MyTemplateh("", DataControlRowType.DataRow);
	...
	protected void FillData(string sortExpression, string direction)
	{
		this.LoadField();
		int recordCount = 0;
		string text = (this.hfParm.Value == "-1") ? this.strParm() : (this.hfParm.Value + " and DisposalID=1 and curStatus='待结算' ");
	...
	{
		string text = " DisposalID=1 and curStatus='待结算' ";
	...
	{
		e.Row.Cells[0].Visible = (e.Row.Cells[2].Visible = (e.Row.Cells[3].Visible = (e.Row.Cells[4].Visible = false)));
		if (this.count == 0)
		{
		...
		{
			e.Row.Cells[this.count + 5].Visible = false;
		}
		...
		if (e.Row.RowType == DataControlRowType.DataRow)
		{
			...
			else if (e.Row.Cells[this.count + 5].Text == "1")
			{
				e.Row.Attributes.Add("style", "color:#893232");
			}
			if (this.blayout)
