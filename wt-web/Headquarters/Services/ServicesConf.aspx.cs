...
public partial class Headquarters_Services_ServicesConf : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "gd_r4"))
				{
	...
	protected void LoadField()
	{
		...
		DataTable dataTable = dALSys.GetLayoutDetail(1, 1, 6, 0, userID).Tables[0];
			...
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 1)
				{
					...
					if (dataField != "ID")
					{
						...
						{
							HiddenField expr_2D0 = this.hfTbTitle;
							expr_2D0.Value = expr_2D0.Value + "," + text2;
						}
						...
						{
							HiddenField expr_320 = this.hfTbField;
							expr_320.Value = expr_320.Value + "," + dataField;
						}
					}
				}
			}
	...
	{
		string text = " DisposalID=1 and curStatus='待确认'";
	...
	protected void btnConf_Click(object sender, EventArgs e)
	{
		...
		if (num2 > 0)
		{
			num = dALServices.ChkConf(num2, int.Parse((string)this.Session["Session_wtUserID"]), out empty);
		}
		if (num == 0)
		{
			this.hfRecID.Value = "-1";
