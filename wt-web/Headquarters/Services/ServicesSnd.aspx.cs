...
public partial class Headquarters_Services_ServicesSnd : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "gd_r10"))
				{
	...
	protected void LoadField()
	{
		...
		DataTable dataTable = dALSys.GetLayoutDetail(1, 1, 8, 0, userID).Tables[0];
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		int recordCount = 0;
		string text = (this.hfParm.Value == "-1") ? this.strParm() : (this.hfParm.Value + " and DisposalID=1 and curStatus='送修' and RepairStatus='待送修发货' ");
			...
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				if (i > 3)
				{
					...
					if (dataField != "ID")
					{
						...
						{
							HiddenField expr_2EB = this.hfTbTitle;
							expr_2EB.Value = expr_2EB.Value + "," + text2;
						}
						...
						{
							HiddenField expr_33B = this.hfTbField;
							expr_33B.Value = expr_33B.Value + "," + dataField;
						}
					}
				}
			}
	...
	{
		string text = " DisposalID=1 and curStatus='送修' and RepairStatus='待送修发货' ";
	...
	protected void btnNO_Click(object sender, EventArgs e)
	{
		...
		{
			DataTable dataTable = DALCommon.GetDataList("fw_services", "", " ProductSN1='" + text + "' and DisposalID=1 and curStatus='送修' and RepairStatus='待送修发货' ").Tables[0];
			if (dataTable.Rows.Count > 0)
			{
	...
	protected void btnSnd_Click(object sender, EventArgs e)
	{
		...
		if (text != "")
		{
			...
			string str = "";
			int num = dALServices.RepairSnd(1, int.Parse((string)this.Session["Session_wtUserID"]), text, int.Parse(this.hfSupID.Value), this.tbSndDate.Text, int.Parse(this.ddlSndStyle.SelectedValue), FunLibrary.ChkInput(this.tbPostNO.Text), postage, this.tbLinkman.Text.Trim(), this.tbAdr.Text.Trim(), out str);
			if (num == 0)
			{
