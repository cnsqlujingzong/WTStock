...
public partial class Headquarters_Stat_ServiceDe3 : Page, IRequiresSessionState
{
	...
	{
		string text = " ID in(select BillID from ServicesProcess where 1=1 ";
		if (this.startdate != "")
		{
			text = text + " and _Date>='" + this.startdate + "' ";
		}
		if (this.enddate != "")
		{
			text = text + " and _Date<='" + this.enddate + " 23:59:59' ";
		}
		if (this.deptid != -1)
		{
			text = text + " and DeptID=" + this.deptid;
		}
		text = text + " and charindex('" + this.strid + "',TakeSteps)>0 ";
		return text + ")";
	}
