...
public partial class Headquarters_Stat_StPayDe : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "tj_r46"))
				{
	...
	{
		int num = 0;
		string text = (this.hfParm.Value == "-1") ? this.strParm() : (this.hfParm.Value + " and Status='已审核' and Type='付款单'");
	...
	protected string strParm()
	{
		...
		string text3 = " Status='已审核' and Type='付款单' ";
		...
		if (this.hfSch.Value == "0")
		{
			...
			{
				if (this.ddlKey.SelectedValue == "_Date")
				{
					text3 = text3.Replace("ChkDate", "_Date");
				}
				if (text4 != "")
				{
					string text5 = text3;
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "payde", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
	...
	protected void ddlKey_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		else if (this.ddlKey.SelectedValue == "ChkDate")
		{
			this.tbCon.Text = string.Empty;
			this.tbCon.Visible = false;
		}
