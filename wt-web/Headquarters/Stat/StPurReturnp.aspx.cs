...
public partial class Headquarters_Stat_StPurReturnp : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "tj_r18"))
				{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvdata.DataSource = DALCommon.GetStData("tj_cg_th", 1, text, strOrderFld);
	...
	protected void gvdata_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.Header)
		{
			...
			cells[2].Text = "退货</th></tr><tr class=\"tdHead8\">";
			cells.Add(new TableHeaderCell());
			cells[3].Text = "编号";
			cells.Add(new TableHeaderCell());
			cells[4].Text = "名称";
	...
	{
		DataTable dt = DALCommon.GetStData("tj_cg_th", 1, this.hfSql.Value, this.hfOrderName.Value + " " + this.hfOrder.Value).Tables[0];
		...
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "退货汇总表", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
