...
public partial class Headquarters_Stat_StPurchaseg : Page, IRequiresSessionState
{
	private decimal dqty = 0m;

	private decimal dtotal = 0m;

	private decimal dtotala = 0m;
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			this.ddlBranch.Items.Insert(0, new ListItem("总部", "1"));
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvdata.DataSource = DALCommon.GetStData("tj_cg_rk", 0, text, strOrderFld);
	...
	protected void gvdata_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.Header)
		{
			...
			cells[2].Text = "采购</th></tr><tr class=\"tdHead8\">";
			cells.Add(new TableHeaderCell());
			cells[3].Text = "编号";
			cells.Add(new TableHeaderCell());
			cells[4].Text = "名称";
		...
		if (e.Row.RowType == DataControlRowType.Footer)
		{
			...
			e.Row.Cells[7].Text = this.dtotala.ToString("#0.00");
		}
	}

	protected void btnExcel_Click(object sender, EventArgs e)
	{
		DataTable dt = DALCommon.GetStData("tj_cg_rk", 0, this.hfSql.Value, this.hfOrderName.Value + " " + this.hfOrder.Value).Tables[0];
		...
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "采购汇总表", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
