...
public partial class Headquarters_Stat_StSerContractDe : Page, IRequiresSessionState
{
	...
	{
		FunLibrary.ChkHead();
		this.conid = base.Request["conid"].Trim();
		...
		if (this.conid == null || this.conid.Trim().Equals(string.Empty))
		{
			base.Response.End();
		}
		else if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "tj_r67"))
				{
	...
	protected void FillData()
	{
		...
		text = text + " and mc.ID=" + this.conid;
		if (this.deptid > 0)
		{
			text = text + " and mc.DeptID=" + this.deptid;
		}
		DataSet dataSet = dALServices.StSerCosts(11, text, strDateS, strDateE);
		...
		boundField3.HeaderText = "设备编号";
		boundField3.DataField = "DeviceNO";
		this.gvdata.Columns.Add(boundField3);
		BoundField boundField4 = new BoundField();
		boundField4.HeaderText = "序列号";
		boundField4.DataField = "ProductSN1";
		...
		for (int i = 0; i < dataSet.Tables[0].Columns.Count; i++)
		{
			...
			boundField6.HeaderText = text4.Substring(1);
			boundField6.DataField = text4;
		...
		boundField7.HeaderText = "备件明细";
		boundField7.DataField = "MaterialDetail";
	...
	protected void gvdata_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.Header)
		{
			...
			cells[3].Text = "序列号";
			cells.Add(new TableHeaderCell());
			cells[4].ColumnSpan = count - 5;
			cells[4].Text = "费用支出";
			cells.Add(new TableHeaderCell());
			cells[5].RowSpan = 2;
			cells[5].Text = "备件明细</th></tr><tr class=\"tdHead3\">";
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		text3 = text3 + " and mc.ID=" + this.conid;
		if (this.deptid > 0)
		{
			text3 = text3 + " and mc.DeptID=" + this.deptid;
		}
		DataTable dataTable = dALServices.StSerCosts(11, text3, text, text2).Tables[0];
		...
		bool flag = false;
		DataToExcel.DataTableToExcel(dataTable, tbTitle, Guid.NewGuid().ToString() + ".xls", "合同序时簿明细", out flag, out empty);
		if (!flag)
		{
			this.FillData();
