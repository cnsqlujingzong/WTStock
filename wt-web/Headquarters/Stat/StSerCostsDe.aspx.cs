...
public partial class Headquarters_Stat_StSerCostsDe : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		int.TryParse(base.Request["sertype"], out this.sertype);
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "tj_r70"))
				{
	...
	protected void FillData()
	{
		...
		if (this.sertype > 0)
		{
			text = text + " and sl.TypeID=" + this.sertype;
		}
		DataSet dataSet = dALServices.StSerCosts(41, text, strDateS, strDateE);
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		if (this.sertype > 0)
		{
			text3 = text3 + " and sl.TypeID=" + this.sertype;
		}
		DataTable dataTable = dALServices.StSerCosts(41, text3, text, text2).Tables[0];
		...
		bool flag = false;
		DataToExcel.DataTableToExcel(dataTable, tbTitle, Guid.NewGuid().ToString() + ".xls", "服务成本明细", out flag, out empty);
		if (!flag)
		{
			this.FillData();
