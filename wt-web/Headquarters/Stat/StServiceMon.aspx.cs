...
public partial class Headquarters_Stat_StServiceMon : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "tj_r29"))
				{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvdata.DataSource = DALCommon.GetStMKBB("tj_fw_report", 1, strStart, strEnd, text, strCondition);
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		DataTable dt = DALCommon.GetStMKBB("tj_fw_report", 1, strStart, strEnd, text, strCondition).Tables[0];
		...
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "服务月报表", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
