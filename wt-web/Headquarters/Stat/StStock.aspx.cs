...
public partial class Headquarters_Stat_StStock : Page, IRequiresSessionState
{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		int iStockID = 0;
		int.TryParse(this.ddlStock.SelectedValue, out iStockID);
		this.gvdata.DataSource = DALCommon.GetStDayData("tj_ck_hz", int.Parse(this.ddlBranch.SelectedValue), now, now2, iStockID);
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		int iStockID = 0;
		int.TryParse(this.ddlStock.SelectedValue, out iStockID);
		DataTable dt = DALCommon.GetStDayData("tj_ck_hz", int.Parse(this.ddlBranch.SelectedValue), now, now2, iStockID).Tables[0];
	...
	{
		if (this.ddlBranch.SelectedValue.ToString() != "-1")
		{
			this.ddlStock.Visible = true;
			this.stock.Visible = true;
			OtherFunction.BindStocks(this.ddlStock, "DeptID=" + int.Parse(this.ddlBranch.SelectedValue));
		}
		else
		{
