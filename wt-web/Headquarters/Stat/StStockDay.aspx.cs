...
public partial class Headquarters_Stat_StStockDay : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "tj_r3"))
				{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvdata.DataSource = DALCommon.GetStDayData("tj_ck_rbb", int.Parse(this.ddlBranch.SelectedValue), now, now2);
	...
	protected void gvdata_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.Header)
		{
			...
			cells[3].Text = "出库</th></tr><tr class=\"tdHead8\">";
			cells.Add(new TableHeaderCell());
			cells[4].Text = "数量";
			cells.Add(new TableHeaderCell());
			cells[5].Text = "金额";
	...
	protected void btnExcel_Click(object sender, EventArgs e)
	{
		...
		DateTime.TryParse(s, out now);
		DateTime.TryParse(s2, out now2);
		DataTable dt = DALCommon.GetStDayData("tj_ck_rbb", int.Parse(this.ddlBranch.SelectedValue), now, now2).Tables[0];
		...
		bool flag = false;
		DataToExcel.DataTableToExcel(dt, tbTitle, Guid.NewGuid().ToString() + ".xls", "仓库日报表", out flag, out empty);
		if (!flag)
		{
			this.FillData(this.hfOrderName.Value, this.hfOrder.Value);
