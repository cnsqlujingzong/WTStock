...
public partial class Headquarters_Stat_StStockINg : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "tj_r5"))
				{
	...
	protected void FillData(string sortExpression, string direction)
	{
			...
			if (selectedValue != null)
			{
				if (!(selectedValue == "1"))
				{
					if (!(selectedValue == "2"))
					{
						...
						{
							text = text + " and a.Attr ='" + this.tbSch.Text + "'";
						}
					}
					else
					{
		...
		this.gvdata.DataSource = DALCommon.GetStData("tj_ck_rk", 0, text, strOrderFld);
		...
		if (this.gvdata.Rows.Count > 0)
		{
			for (int i = 0; i < this.gvdata.HeaderRow.Cells.Count; i++)
			{
				...
				{
					HiddenField expr_37B = this.hfTbTitle;
					expr_37B.Value = expr_37B.Value + "," + text2;
				}
				...
				{
					HiddenField expr_3CB = this.hfTbField;
					expr_3CB.Value = expr_3CB.Value + "," + dataField;
				}
			}
		}
	}
	...
	{
		DataTable dt = DALCommon.GetStData("tj_ck_rk", 0, this.hfSql.Value, this.hfOrderName.Value + " " + this.hfOrder.Value).Tables[0];
