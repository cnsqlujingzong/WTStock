...
public partial class Headquarters_Stat_StStockOUTs : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "tj_r9"))
				{
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		this.gvdata.DataSource = DALCommon.GetStData("tj_ck_ck", 1, text, strOrderFld);
	...
	{
		DataTable dt = DALCommon.GetStData("tj_ck_ck", 1, this.hfSql.Value, this.hfOrderName.Value + " " + this.hfOrder.Value).Tables[0];
