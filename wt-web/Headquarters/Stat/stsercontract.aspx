<%@ page language="C#"  CodeFile="stsercontract.aspx.cs"     autoeventwireup="true" inherits="Headquarters_Stat_StSerContract" theme="themes" enableEventValidation="false" %>
...
<body onload="Chkset();" onclick="parent.Sethidden1();">
    ...
    <asp:GridView ID="gvdata" runat="server" AutoGenerateColumns="false" SkinID="statf" OnRowDataBound="gvdata_RowDataBound" EnableViewState="false" ShowFooter="true">
        ...
        <asp:BoundField HeaderText="维修合同号" DataField="ContractNO" />
        <asp:BoundField HeaderText="合同金额" DataField="dAmount" />
        <asp:BoundField HeaderText="备件成本" DataField="MaterialCost" />
        </Columns>
    </asp:GridView>
    ...
    <asp:HiddenField ID="hfTbTitle" runat="server" Value="维修合同号,合同金额,备件成本," />
    <asp:HiddenField ID="hfTbField" runat="server" Value="GoodsNO,_Name,Spec,Unit,BeginQty,BeginTotal,INQty,INTotal,OUTQty,OUTCost,EndQty,EndTotal" />
...
function ChkView(contractid)
{
    var dates=$("tbDateB").value;
    var datee=$("tbDateE").value;
    var dept=$("ddlBranch").value;
    parent.ShowDialog(800, 480, 'Stat/StSerContractDe.aspx?conid='+contractid+'&deptid='+dept+'&startdate='+dates+'&enddate='+datee, '明细');
}
function Chkset()
{
    Chkwh();
