...
public partial class Headquarters_Stock_AllocateAdd : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "ck_r80"))
				{
					base.Response.Redirect("~/Headquarters/Pur.aspx?p=80");
					base.Response.End();
				}
				if (dALRight.GetRight(num, "ck_r81"))
				{
					this.hfReg.Value = "ck_r81";
				}
			}
			...
			OtherFunction.BindStocks(this.ddlStockOut, " DeptID=1 and bStop=0 ");
			OtherFunction.BindStocks(this.ddlStockIn, " DeptID=1 and bStop=0 ");
		...
		else
		{
			...
			if (dataTable.Rows.Count > 0)
			{
				...
				for (int i = 0; i < dataTable.Rows.Count; i++)
				{
					...
					DataTable dataTable2 = DALCommon.GetGoodsPriceList(3, 1, iGoodsid, iUnitid, 0, int.Parse(this.ddlStockOut.SelectedValue)).Tables[0];
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num == 0)
			{
				...
				dALInnerAllocate.ChkInnerStockOut(1, iTbid, iOperator, out empty);
	...
	protected int BillAdd(out int iTbid)
	{
		...
		innerAllocateInfo.PersonID = int.Parse((string)this.Session["Session_wtUserID"]);
