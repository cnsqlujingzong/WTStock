...
public partial class Headquarters_Stock_BroughtBack : Page, IRequiresSessionState
{
	private int flag;
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (this.flag == 2)
			{
				...
				this.ddlReason.Items.FindByValue("2").Selected = true;
			}
		}
	}
				...
				if (this.hfType.Value == "2")
				{
					...
					if (dataTable.Rows.Count > 0)
					{
						for (int i = 0; i < dataTable.Rows.Count; i++)
						{
							...
							DataTable dataTable2 = DALCommon.GetGoodsPriceList(3, 1, iGoodsid, iUnitid, 0, int.Parse(this.ddlStock.SelectedValue)).Tables[0];
							...
							if (flag)
							{
								...
								dataRow[14] = dataTable.Rows[i]["Remark"].ToString();
								int num2 = 0;
								int.TryParse(dataTable.Rows[i]["StockID"].ToString(), out num2);
								dataRow[15] = num2;
								dataRow[16] = int.Parse(dataTable.Rows[i]["ID"].ToString());
								dataRow[17] = int.Parse(dataTable.Rows[i]["GoodsUnitID"].ToString());
								dataRow[18] = 0;
					...
					if (dataTable.Rows.Count > 0)
					{
						...
						for (int i = 0; i < dataTable.Rows.Count; i++)
						{
							...
							if (flag)
							{
								...
								dataRow[17] = int.Parse(dataTable.Rows[i]["UnitID"].ToString());
								dataRow[18] = int.Parse(dataTable.Rows[i]["ID"].ToString());
								dataRow[19] = dataTable.Rows[i]["CustomerName"].ToString();
								this.tbAQty.Text = Convert.ToDouble(decimal.Parse(this.tbAQty.Text) + decimal.Parse(dataRow[6].ToString())).ToString("#0.00");
								this.tbAmount.Text = Convert.ToDouble(decimal.Parse(this.tbAmount.Text) + decimal.Parse(dataRow[9].ToString())).ToString("#0.00");
								gridViewSource.Rows.Add(dataRow);
							}
	...
	protected void ddlReason_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		{
			OtherFunction.BindStocks(this.ddlStock, " DeptID=1 and bStop=0 and bReject=1 ");
			this.lbOperator.Text = "退";
			this.lbStock.Text = "入库";
		}
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num == 0)
			{
				...
				dALBroughtBack.ChkBroughtBack(1, iTbid, iOperator, out empty);
	...
	protected int BillAdd(out int iTbid)
	{
		...
		broughtBackInfo.DeptID = new int?(1);
		...
		broughtBackInfo.PersonID = new int?(int.Parse(this.Session["Session_wtUserID"].ToString()));
		...
		if (gridViewSource.Rows.Count > 0)
		{
			...
			for (int i = 0; i < gridViewSource.Rows.Count; i++)
			{
				...
				{
					list.Add(new BroughtBackDetailInfo
					{
						StockID = new int?(int.Parse(this.ddlStock.SelectedValue)),
						...
						CiteID = int.Parse(gridViewSource.Rows[i]["ID"].ToString()),
						OperationID = gridViewSource.Rows[i]["BillID"].ToString(),
