...
public partial class Headquarters_Stock_BroughtM : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				{
					Headquarters_Stock_BroughtM.isLowPricePower = false;
				}
			}
			...
			DataTable dataTable = DALCommon.GetDataList("ck_servicesmaterial", "", " Qty<>LQty and OutSourcing=0 and OperationID=" + this.id.ToString()).Tables[0];
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		if (this.CollectData())
		{
			...
			else
			{
				...
				if (num == 0)
				{
					if (!Headquarters_Stock_BroughtM.isLowPricePower)
					{
						if (!flag)
						{
