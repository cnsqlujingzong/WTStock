...
public partial class Headquarters_Stock_DeptStock : Page, IRequiresSessionState
{
	...
	protected void LoadClass()
	{
		...
		Headquarters_Stock_DeptStock.BindTreeNode(treeNode, dt, treeNode.Value);
	...
	public static void BindTreeNode(TreeNode node, DataTable dt, string parentid)
	{
		...
		for (int i = 0; i < array2.Length; i++)
		{
			...
			Headquarters_Stock_DeptStock.BindTreeNode(treeNode, dt, dataRow["ID"].ToString());
		}
	}
	...
	protected void FillData(string sortExpression, string direction)
	{
		...
		int num = 0;
		int.TryParse((string)this.Session["Session_wtUserID"], out num);
		int num2 = int.Parse((string)this.Session["Session_wtPurID"]);
		if (num2 > 0)
		{
			DALRight dALRight = new DALRight();
			if (dALRight.GetRight(num2, "ck_r82"))
			{
				object obj = text;
				text = string.Concat(new object[]
				{
					...
					num,
					" or charindex('",
					this.Session["Session_wtUser"].ToString(),
					"',OtherStockAdm)>0) "
				});
			}
		}
