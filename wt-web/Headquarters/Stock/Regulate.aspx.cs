...
public partial class Headquarters_Stock_Regulate : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			...
			if (num > 0)
			{
				...
				if (!dALRight.GetRight(num, "ck_r53"))
				{
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num == 0)
			{
				...
				dALRegulate.ChkRegulate(1, iTbid, iOperator, out empty);
	...
	protected int BillAdd(out int iTbid)
	{
		...
		int personID = 0;
		int.TryParse((string)this.Session["Session_wtUserID"], out personID);
