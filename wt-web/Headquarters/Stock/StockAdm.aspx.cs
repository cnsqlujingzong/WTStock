...
// Type: Headquarters_Stock_StockAdm
...
public partial class Headquarters_Stock_StockAdm : Page, IRequiresSessionState
{
    ...
    protected void LoadClass()
    {
        ...
        Headquarters_Stock_StockAdm.BindTreeNode(treeNode, dt, treeNode.Value);
    ...
    public static void BindTreeNode(TreeNode node, DataTable dt, string parentid)
    {
        foreach (DataRow dataRow in dt.Select(" Father=" + parentid))
        {
            ...
            Headquarters_Stock_StockAdm.BindTreeNode(treeNode, dt, dataRow["ID"].ToString());
        }
    }
    ...
    protected void LoadField()
    {
        ...
        DataTable dataTable = new DALSys().GetLayoutDetail(1, 1, 2, 0, result).Tables[0];
    ...
    protected void GetStockDept()
    {
        ...
        int.TryParse((string)this.Session["Session_wtUserID"], out result);
        int RightID = int.Parse((string)this.Session["Session_wtPurID"]);
        if (RightID > 0 && new DALRight().GetRight(RightID, "ck_r82"))
            str = str + (object)" and (StockAdmID1=" + (string)(object)result + " or StockAdmID2=" + (string)(object)result + " or charindex('" + this.Session["Session_wtUser"].ToString() + "',OtherStockAdm)>0) ";
