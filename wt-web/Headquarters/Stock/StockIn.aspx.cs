...
public partial class Headquarters_Stock_StockIn : Page, IRequiresSessionState
{
	...
	protected void gvdata_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if (e.Row.RowType == DataControlRowType.DataRow)
		{
			...
			e.Row.Cells[9].Attributes.Add("ondblclick", "SltUnit();");
			...
			e.Row.Cells[10].Attributes.Add("ondblclick", "SltUnitQty('" + textBox.ClientID + "');");
	...
	protected void btnChk_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			if (num == 0)
			{
				...
				dALStockIN.ChkStockIn(1, iTbid, iOperator, out empty);
	...
	protected int BillAdd(out int iTbid)
	{
		...
		stockINInfo.PersonID = new int?(int.Parse(this.Session["Session_wtUserID"].ToString()));
		stockINInfo.Remark = FunLibrary.ChkInput(this.tbRemark.Text);
        stockINInfo.SendMoney = FunLibrary.ChkInput(this.tb_famonty.Text);
        stockINInfo.SendName = FunLibrary.ChkInput(this.ddl_fatype.SelectedValue);
        stockINInfo.SendNum = FunLibrary.ChkInput(this.tb_fanum.Text);
