<%@ page language="C#"  CodeFile="goodsadd.aspx.cs"     autoeventwireup="true" inherits="Headquarters_Stock_GoodsAdd" theme="Themes" enableEventValidation="false" %>
...
function ChkAdd()
{
    ...
    if (isNull($("tbSpec").value)) {
        window.alert("操作失败！产品规格不能为空");
        $("tbSpec").focus();
        return false
    } if (isNull($("tbBrand").value)) {
        window.alert("操作失败！产品品牌不能为空");
        $("tbBrand").focus();
        return false
    }
    if($("ddlCostMode").value=="")
    {
        ChkTab('1');
        $("ddlCostMode").focus();
        window.alert("操作失败！成本模式不能为空");
        return false
    }
