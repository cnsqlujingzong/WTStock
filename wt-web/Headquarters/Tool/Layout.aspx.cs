...
public partial class Headquarters_Tool_Layout : Page, IRequiresSessionState
{
	...
	{
		int num = 0;
		DataTable dt = DALCommon.GetList_HL(0, "LayoutClass", "", 0, 0, " bSys=0 and DeptID=1 ", " ID Asc", out num).Tables[0];
		this.tv.Nodes.Clear();
	...
	public void FillData()
	{
		...
		{
			DataTable dataTable = dALSys.GetLayoutDetail(0, 1, int.Parse(this.ddlWinName.SelectedValue), classID, 0).Tables[0];
		...
		{
			this.GridView1.DataSource = dALSys.GetLayoutDetail(0, 1, int.Parse(this.ddlWinName.SelectedValue), classID, 0).Tables[0];
			this.GridView1.DataBind();
		}
	}
	...
	protected void btnBackDefault_Click(object sender, EventArgs e)
	{
		...
		else
		{
			...
			int num2 = DALCommon.DeteleData("LayoutDetail", " DeptID=1 and ClassID=" + this.hfClass.Value + " and WinName=" + this.ddlWinName.SelectedValue, out empty);
			if (num2 == 0)
			{
	...
	protected void btnSave_Click(object sender, EventArgs e)
	{
		...
		{
			DALSys dALSys = new DALSys();
			DataTable dataTable = DALCommon.GetDataList("LayoutDetail", "ID", " DeptID=1 and ClassID=" + num.ToString() + " and WinName=" + this.ddlWinName.SelectedValue).Tables[0];
