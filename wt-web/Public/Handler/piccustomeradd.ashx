<%@ WebHandler Language="C#" Class="PicCustomerAdd" %>
...
public class PicCustomerAdd : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        DataTable dt = DALCommon.GetDataList("CustomerList", "_Date", parm + " and (_Date>='" + year + "-1-1 00:00:00' and _Date<'" + year + "-12-31 23:59:59') ").Tables[0];
        if (dt.Rows.Count > 0)
        {
            DateTime date = DateTime.Now;
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DateTime.TryParse(dt.Rows[i]["_Date"].ToString(), out date);
        ...
        //标题
        g.DrawString("客户增长情况(" + year + "年)", new Font("黑体", 14, FontStyle.Regular), new SolidBrush(Color.Black), new PointF(400, 15));
        ...
        for (int i = 0; i <= 11; i++)
        {
            ...
            g.DrawRectangle(pen, i * 66 + 74, 435 - (iheight), 40, (iheight) + 1);
            g.FillRectangle(brush, i * 66 + 75, 436 - (iheight), 39, iheight);
        }
