<%@ WebHandler Language="C#" Class="PicIncomeType" %>
...
public class PicIncomeType : IHttpHandler, IRequiresSessionState
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (idept)
        {
            ...
            default:
                {
                    parm += " and DeptID=" + idept.ToString();
                } break;
        }

        DataTable tb_type = DALCommon.GetDataList("zk_incomeexpenses", " ChargeStyle,count(1) as _Count", parm + " and ChkDate>='" + year + "-01-01 00:00:00' and ChkDate<'" + year + "-12-31 23:59:59'  group by ChargeStyle").Tables[0];
        ...
        //标题
        g.DrawString("收款方式(" + year + "年)", new Font("黑体", 14, FontStyle.Regular), new SolidBrush(Color.Black), new PointF(380, 15));
        ...
        for (int i = 0; i < count_type; i++)
        {
            ...
            brush.Color = Color.Black;
            g.DrawString(tb_type.Rows[i]["ChargeStyle"].ToString(), new Font("宋体", 9, FontStyle.Regular), brush, new PointF(872, 11 + i * 20));

            x_value[i] = float.Parse(tb_type.Rows[i]["_Count"].ToString());
            x_text[i] = tb_type.Rows[i]["ChargeStyle"].ToString();
