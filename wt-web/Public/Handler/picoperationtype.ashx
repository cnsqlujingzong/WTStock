<%@ WebHandler Language="C#" Class="PicOperationType" %>
...
public class PicOperationType : IHttpHandler, IRequiresSessionState
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        DataTable tb_type = DALCommon.GetDataList("fw_services", " ServicesType,count(1) as _Count", parm + " and Time_TakeOver>='" + year + "-01-01 00:00:00' and Time_TakeOver<'" + year + "-12-31 23:59:59'  group by ServicesType").Tables[0];
        ...
        //标题
        g.DrawString("服务类别(" + year + "年)", new Font("黑体", 14, FontStyle.Regular), new SolidBrush(Color.Black), new PointF(380, 15));
        ...
        for (int i = 0; i < count_type; i++)
        {
            ...
            brush.Color = Color.Black;
            g.DrawString(tb_type.Rows[i]["ServicesType"].ToString(), new Font("宋体", 9, FontStyle.Regular), brush, new PointF(872, 11 + i * 20));

            x_value[i] = float.Parse(tb_type.Rows[i]["_Count"].ToString());
            x_text[i] = tb_type.Rows[i]["ServicesType"].ToString();
