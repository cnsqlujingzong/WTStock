<%@ WebHandler Language="C#" Class="PicSatisCusClass" %>
using System;
using System.Web;
...
public class PicSatisCusClass : IHttpHandler, IRequiresSessionState
{
    public void ProcessRequest(HttpContext context)
    {
        //收集数据
        string year_b = context.Request["timeb"];
        string year_e = context.Request["timee"];
        string widths = context.Request["width"];
        string heights = context.Request["height"];
        string dept = context.Request["dept"];

        year_b = wt.Library.FunLibrary.ChkInput(year_b);
        year_e = wt.Library.FunLibrary.ChkInput(year_e);
        ...
        if (dept != "-1")
            parm = parm + " and RegDept=" + dept.ToString();

        DateTime dt_b = DateTime.Now;
        if (year_b != string.Empty) { DateTime.TryParse(year_b, out dt_b); }
        DateTime dt_e = DateTime.Now;
        if (year_e != string.Empty) { DateTime.TryParse(year_e, out dt_e); }

        DataTable tb_type = DALCommon.GetDataList("ks_customer", " ClassName,count(1) as _Count", parm + " and _Date>='" + dt_b.ToString("yyyy-MM-dd") + " 00:00:00' and _Date<'" + dt_e.ToString("yyyy-MM-dd") + " 23:59:59' group by ClassName").Tables[0];
        ...
        //标题
        g.DrawString("客户类别分析(" + dt_b.ToString("yyyy-MM-dd") + "到" + dt_e.ToString("yyyy-MM-dd") + ")", new Font("黑体", 14, FontStyle.Regular), new SolidBrush(Color.Black), new PointF(280, 15));
        ...
        for (int i = 0; i < count_type; i++)
        {
            ...
            brush.Color = Color.Black;
            g.DrawString(tb_type.Rows[i]["ClassName"].ToString(), new Font("宋体", 9, FontStyle.Regular), brush, new PointF(872, 11 + i * 20));

            x_value[i] = float.Parse(tb_type.Rows[i]["_Count"].ToString());
            x_text[i] = tb_type.Rows[i]["ClassName"].ToString();
