<%@ WebHandler Language="C#" Class="PicSatisCusFrom" %>
using System;
using System.Web;
...
public class PicSatisCusFrom : IHttpHandler, IRequiresSessionState
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        DataTable tb_type = DALCommon.GetDataList("ks_customer", " CusFrom,count(1) as _Count", parm + " and _Date>='" + dt_b.ToString("yyyy-MM-dd") + " 00:00:00' and _Date<'" + dt_e.ToString("yyyy-MM-dd") + " 23:59:59' group by CusFrom").Tables[0];
        ...
        //标题
        g.DrawString("客户来源分析(" + dt_b.ToString("yyyy-MM-dd") + "到" + dt_e.ToString("yyyy-MM-dd") + ")", new Font("黑体", 14, FontStyle.Regular), new SolidBrush(Color.Black), new PointF(280, 15));
        ...
        for (int i = 0; i < count_type; i++)
        {
            ...
            brush.Color = Color.Black;
            g.DrawString(tb_type.Rows[i]["CusFrom"].ToString(), new Font("宋体", 9, FontStyle.Regular), brush, new PointF(872, 11 + i * 20));

            x_value[i] = float.Parse(tb_type.Rows[i]["_Count"].ToString());
            x_text[i] = tb_type.Rows[i]["CusFrom"].ToString();
