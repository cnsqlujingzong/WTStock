<%@ WebHandler Language="C#" Class="PicTakeStepsDis" %>
...
public class PicTakeStepsDis : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context) 
    {
        ...
        if (deptid != "-1")
        {
            parm += " and DeptID=" + deptid;
        }
        if (startdate != "")
        {
            parm += " and convert(char(10),_Date,120)>='" + startdate + "'";
        }
        if (enddate != "")
        {
            parm += " and convert(char(10),_Date,120)<='" + enddate + " 23:59:59'";
        }
        ...
        //标题
        g.DrawString("处理措施分布图", new Font("黑体", 14, FontStyle.Regular), new SolidBrush(Color.Black), new PointF(250, 15));
        ...
        for (int i = 0; i < count_type; i++)
        {
            ...
            brush.Color = Color.Black;
            g.DrawString(tb_type.Rows[i]["Summary"].ToString(), new Font("宋体", 9, FontStyle.Regular), brush, new PointF(622, 11 + i * 20));

            x_value[i] = float.Parse(tb_type.Rows[i]["TakeStepsCount"].ToString());
            x_text[i] = tb_type.Rows[i]["Summary"].ToString();
