<%@ WebHandler Language="C#" Class="PicTakeType" %>
...
public class PicTakeType : IHttpHandler, IRequiresSessionState
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        DataTable tb_type = DALCommon.GetDataList("fw_services", " TakeStyle,count(1) as _Count", parm + " and Time_TakeOver>='" + year + "-01-01 00:00:00' and Time_TakeOver<'" + year + "-12-31 23:59:59'  group by TakeStyle").Tables[0];
        ...
        //标题
        g.DrawString("受理方式(" + year + "年)", new Font("黑体", 14, FontStyle.Regular), new SolidBrush(Color.Black), new PointF(380, 15));
        ...
        for (int i = 0; i < count_type; i++)
        {
            ...
            brush.Color = Color.Black;
            g.DrawString(tb_type.Rows[i]["TakeStyle"].ToString(), new Font("宋体", 9, FontStyle.Regular), brush, new PointF(872, 11 + i * 20));

            x_value[i] = float.Parse(tb_type.Rows[i]["_Count"].ToString());
            x_text[i] = tb_type.Rows[i]["TakeStyle"].ToString();
